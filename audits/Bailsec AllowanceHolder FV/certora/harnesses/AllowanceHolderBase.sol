...
import {SafeTransferLib} from "src/vendor/SafeTransferLib_Solmate.sol";
import {CheckCall} from "./CheckCall.sol";
import {FreeMemory} from "src/utils/FreeMemory.sol";
...
abstract contract AllowanceHolderBase is TransientStorageLayout, FreeMemory {
    ...
    //@certora function split in _rejectIfERC20 and _rejectIfERC20Inner because DANGEROUS_freeMemory modifier
    // can not be modeled by certora (calls the memory pointer). This allows a summary of the function to test the inner code
    //@certora changed function to internal virtual to be able to override in the harness => bypass the DANGEROUS_freeMemory modifier
    //ORIGINAL: function _rejectIfERC20(address payable maybeERC20, bytes calldata data) private view DANGEROUS_freeMemory {
    function _rejectIfERC20(address payable maybeERC20, bytes calldata data) internal virtual view DANGEROUS_freeMemory {
        _rejectIfERC20Inner(maybeERC20, data);
    }

    //@certora helper function to be able to test the inner code without the DANGEROUS_freeMemory modifier
    //         return value added to be able to use the target in rules
    function _rejectIfERC20Inner(address payable maybeERC20, bytes calldata data) internal view returns (address) {
        ...
        //@certora return value added to be able to use the target in rules
        return target; 
    }

    //@certora private -> internal to be able to summarize the function
    //@certora ORIGINAL: function _msgSender() private view returns (address sender) {
    function _msgSender() internal view returns (address sender) {
        if ((sender = msg.sender) == address(this)) {
            //ORIGINAL:assembly (memory-safe) {
                // sender := shr(0x60, calldataload(sub(calldatasize(), 0x14)))
            //@certora replaced the assembly block with the Solidity-level call
            sender = _msgSenderSol();
        }
    }

    //@certora helper function for the _msgSender function
    function _msgSenderSol() internal pure returns (address) {
