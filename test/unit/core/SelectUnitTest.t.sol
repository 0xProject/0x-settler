...
contract SelectUnitTest is Permit2Signature, DeployPermit2 {
    ...
    /// @dev With ample transaction gas, this cap keeps every trial fully funded.
    ...
    function test_allTargetsMiss_revertsLastShortfall() public {
    ...
    function test_firstCandidateSuccess_skipsRest() public {
    ...
    function test_nestedSelect_uncappedOuterFallbackCanFundInnerSelect() public {
        p0.set(7 ether, false);
        ...
        assertEq(p0.callCount(), 0, "an equal-capped outer trial cannot fully fund an inner trial");
...
/// @dev Separate contract: SelectUnitTest sits at the unit-profile deploy-gas ceiling.
contract SelectGasFundingTest is Permit2Signature, DeployPermit2 {
    BaseSettler internal settler;
    MockERC20 internal buy;
    Pool internal p0;
    Pool internal p1;
    ...
    /// @dev Below `_SELECT_OVERHEAD_GAS + GAS_CAP + GAS_CAP / 63`: a trial at this budget starves.
    uint256 internal constant STARVING_TX_GAS = 300_000;
    ...
    function _selectCallData(bytes[][] memory candidates) internal view returns (bytes memory) {
        bytes memory action = abi.encodeCall(
            ISettlerActions.SELECT, (GAS_CAP, address(buy), new uint256[](candidates.length), candidates)
        );
    ...
    function _run(bytes[][] memory candidates, uint256 txGas)
        internal
        returns (bool success, bytes memory returndata, uint256 consumed)
    {
        bytes memory callData = _selectCallData(candidates);
        vm.prank(taker, taker);
        uint256 beforeGas = gasleft();
        (success, returndata) = address(settler).call{gas: txGas}(callData);
        consumed = beforeGas - gasleft();
    }

    function test_starvedTrialSuccess_commits() public {
        p0.set(8 ether, false);
        bytes[][] memory candidates = new bytes[][](2);
        candidates[0] = _candidate(address(p0));
        candidates[1] = _candidate(address(p1));

        (bool success,,) = _run(candidates, STARVING_TX_GAS);

        assertTrue(success, "starved first trial succeeded");
        assertEq(buy.balanceOf(recipient), 8 ether, "starved success committed");
        assertEq(p1.callCount(), 0, "no fallback attempted");
    }

    function test_starvedTrialFailure_consumesAllGas() public {
        p0.set(8 ether, true);
        bytes[][] memory candidates = new bytes[][](2);
        candidates[0] = _candidate(address(p0));
        candidates[1] = _candidate(address(p1));

        (bool success, bytes memory returndata, uint256 consumed) = _run(candidates, STARVING_TX_GAS);

        assertFalse(success, "possibly starved failure reverted everything");
        assertEq(returndata.length, 0, "an all-gas burn carries no returndata");
        assertGt(consumed, STARVING_TX_GAS - 1_000, "the whole allotment burned");
        assertEq(p1.callCount(), 0, "the failure did not fall through");
    }

    function test_starvedLastTrialFailure_consumesAllGasInsteadOfBubbling() public {
        p1.set(8 ether, true);
        bytes[][] memory candidates = new bytes[][](2);
        candidates[0] = _candidate(address(new GasHeavyPool(IERC20(address(buy)), 0, type(uint256).max)));
        candidates[1] = _candidate(address(p1));

        (bool success, bytes memory returndata, uint256 consumed) = _run(candidates, 600_000);

        assertFalse(success, "starved last trial reverted everything");
        assertEq(returndata.length, 0, "burned instead of bubbling the candidate's revert");
        assertGt(consumed, 590_000, "the whole allotment burned");
    }
}
