...
abstract contract SettlerPairTest is SettlerBasePairTest {
    ...
    function testSettler_uniswapV3_buyToken_fee_full_custody() public skipIf(uniswapV3Path().length == 0) {
        ...
        _settler.execute(
            SettlerBase.AllowedSlippage({recipient: FROM, buyToken: toToken(), minAmountOut: 0 ether}),
            actions,
            bytes32(0)
        );
        snapEnd();
    ...
    function testSettler_basic_curve() public skipIf(getCurveV2PoolData().pool == address(0)) {
        ...
        _settler.execute(
            SettlerBase.AllowedSlippage({buyToken: toToken(), recipient: FROM, minAmountOut: 0 ether}),
            actions,
            bytes32(0)
        );
        snapEnd();
