...
reduction keeps `|t| < ln2/2 · 2^128`). From that bound this file transports the downstream kernel
stages to closed `Int`/bound forms:

* `v = t²` in Q128 (a nonnegative logical shift, `< 2^126`);
* the even/odd Horner accumulators `ev`, `od` (two-sided constant bounds);
...
The octave reduction couples `k` to `x` (`2^200·k ≈ CINV·x`), so the residual `K27·x − LN2·k`
...
theorem tTree_bound {x : Nat} (hx : x < 2 ^ 256)
  ...
  rw [p127]
  -- Eliminate `k` by scaling both sandwiches to the common factor `2^200`, then bound `X`.
  ...
  have hklo' : 38271408169742254668347313025622401492114385419650052359639581444463709 *
      (1606938044258990275541962092341162602522202993782792835301376 * k) ≤
      38271408169742254668347313025622401492114385419650052359639581444463709 *
      (803469022129495137770981046170581301261101496891396417650688 +
        2318321547468254865173387471183990 * X) :=
  ...
  -- 2^200 times the t-sandwich:
  have hp200pos : (0 : Int) < 1606938044258990275541962092341162602522202993782792835301376 := by
    norm_num
  have htlo' : 1606938044258990275541962092341162602522202993782792835301376 *
  ...
  have hthi' : 1606938044258990275541962092341162602522202993782792835301376 *
      (55213970774324510299478046898216203619608872 * X -
        38271408169742254668347313025622401492114385419650052359639581444463709 * k) <
      1606938044258990275541962092341162602522202993782792835301376 *
...
/-- The Q128 square `v = ⌊t²/2^128⌋` as a `Nat`: nonnegative, and `< 2^126`. The shift argument
`t·t` fits in a word because `|t| < 2^127` gives `t² < 2^254`. -/
theorem vTree_eq {x : Nat} (hx : x < 2 ^ 256)
    (hC : int256 Cmask < int256 x) (hC0 : int256 x < int256 C0thresh) :
    (vTree x : Int) = (int256 (tTree x))^2 / 2 ^ 128 ∧ vTree x < 2 ^ 126 := by
  ...
  have hsq_lt : t ^ 2 < 2 ^ 254 := by
    have hp127 : (2:Int)^127 = 170141183460469231731687303715884105728 := by norm_num
    have hp254 : (2:Int)^254 = 28948022309329048855892746252171976963317496166410141009864396001978282409984 := by norm_num
    rw [hp127] at htlo hthi
    rw [hp254, sq]
    nlinarith [htlo, hthi]
  have hsq_nn : 0 ≤ t ^ 2 := by positivity
  -- `tTree x · tTree x` as a word equals `t²` (transport), nonneg, `< 2^254`.
  ...
  -- its `int256` is nonneg and below `2^254`, so it is the literal Nat value
  ...
  have hmul_nat_lt : evmMul (tTree x) (tTree x) < 2 ^ 254 := by
    have : ((evmMul (tTree x) (tTree x) : Nat) : Int) < 2 ^ 254 := by
      rw [hmul_nat, ← sq]; exact hsq_lt
    exact_mod_cast this
  refine ⟨?_, ?_⟩
  · unfold vTree
    rw [evmShr_eq_div (by norm_num) hmul_lt]
    have he : ((evmMul (tTree x) (tTree x) / 2 ^ 128 : Nat) : Int) =
        (evmMul (tTree x) (tTree x) : Int) / 2 ^ 128 := by
      rw [Int.ofNat_ediv]; norm_num
    rw [he, hmul_nat, ← sq]
  · unfold vTree
    rw [evmShr_eq_div (by norm_num) hmul_lt]
    have : evmMul (tTree x) (tTree x) / 2 ^ 128 < 2 ^ 254 / 2 ^ 128 :=
      Nat.div_lt_div_of_lt_of_dvd (by norm_num) hmul_nat_lt
    have he : (2:Nat) ^ 254 / 2 ^ 128 = 2 ^ 126 := by
...
The bounds chain from `v < 2^126` through the five even / four odd stages. -/
...
theorem ev0_lt {v : Nat} (hv : v < 2 ^ 126) :
    evmAdd 0xb9aacfad41060587203a79af0ebc (evmShr 0x1d v) < 2 ^ 113 := by
  have hsh0 : evmShr 0x1d v = v / 2 ^ 0x1d := evmShr_eq_div (by norm_num) (by omega)
  have hev0t : v / 2 ^ 0x1d < 2 ^ 97 := by
    ...
    omega
  rw [hsh0, evmAdd_eq_nat (by norm_num) (by omega) (by omega)]; omega

theorem ev0_ge {v : Nat} (hv : v < 2 ^ 126) :
    0xb9aacfad41060587203a79af0ebc ≤ evmAdd 0xb9aacfad41060587203a79af0ebc (evmShr 0x1d v) := by
  have hsh0 : evmShr 0x1d v = v / 2 ^ 0x1d := evmShr_eq_div (by norm_num) (by omega)
  have hev0t : v / 2 ^ 0x1d < 2 ^ 97 := by
    have : v / 2 ^ 0x1d < 2 ^ 126 / 2 ^ 0x1d := Nat.div_lt_div_of_lt_of_dvd (by norm_num) hv
    have he : (2:Nat) ^ 126 / 2 ^ 0x1d = 2 ^ 97 := by rw [Nat.pow_div (by norm_num) (by norm_num)]
...
/-- Two-sided bound on the even Horner accumulator: `0x4e14… ≤ ev < 2^127`. -/
theorem evTree_facts {x : Nat} (hv : vTree x < 2 ^ 126) :
    0x4e14a45e8ec305e233e11b4174e214ac ≤ evTree x ∧ evTree x < 2 ^ 127 := by
  have hev : evTree x =
  ...
  have h0 := ev0_lt hv
  set ev0 := evmAdd 0xb9aacfad41060587203a79af0ebc (evmShr 0x1d v) with hev0
  have h1 : evmAdd 0x9a036222e11aee18465042f8ea64c8 (evmShr 0x82 (evmMul ev0 v)) < 2 ^ 121 := by
    have := (stage_bounds (c := 0x9a036222e11aee18465042f8ea64c8) (prev := ev0) (v := v)
      (P := 2 ^ 113) (V := 2 ^ 126) (sh := 0x82) h0 hv (by norm_num) (by norm_num)
      (by rw [pvd 113 126 130 109 (by norm_num)]; norm_num)).2
    rw [pvd 113 126 130 109 (by norm_num)] at this; omega
  set ev1 := evmAdd 0x9a036222e11aee18465042f8ea64c8 (evmShr 0x82 (evmMul ev0 v)) with hev1
  have h2 : evmAdd 0x9064d965e1c4863b73604e0ddbec53f9 (evmShr 0x80 (evmMul ev1 v)) < 2 ^ 129 := by
    have := (stage_bounds (c := 0x9064d965e1c4863b73604e0ddbec53f9) (prev := ev1) (v := v)
      (P := 2 ^ 121) (V := 2 ^ 126) (sh := 0x80) h1 hv (by norm_num) (by norm_num)
      (by rw [pvd 121 126 128 119 (by norm_num)]; norm_num)).2
    rw [pvd 121 126 128 119 (by norm_num)] at this; omega
  set ev2 := evmAdd 0x9064d965e1c4863b73604e0ddbec53f9 (evmShr 0x80 (evmMul ev1 v)) with hev2
  have h3 : evmAdd 0x93f11e65781741b92fa7fc4f4fffcca2 (evmShr 0x86 (evmMul ev2 v)) < 2 ^ 129 := by
    have := (stage_bounds (c := 0x93f11e65781741b92fa7fc4f4fffcca2) (prev := ev2) (v := v)
      (P := 2 ^ 129) (V := 2 ^ 126) (sh := 0x86) h2 hv (by norm_num) (by norm_num)
      (by rw [pvd 129 126 134 121 (by norm_num)]; norm_num)).2
    rw [pvd 129 126 134 121 (by norm_num)] at this; omega
  set ev3 := evmAdd 0x93f11e65781741b92fa7fc4f4fffcca2 (evmShr 0x86 (evmMul ev2 v)) with hev3
  have hfin := stage_bounds (c := 0x4e14a45e8ec305e233e11b4174e214ac) (prev := ev3) (v := v)
    (P := 2 ^ 129) (V := 2 ^ 126) (sh := 0x84) h3 hv (by norm_num) (by norm_num)
    (by rw [pvd 129 126 132 123 (by norm_num)]; norm_num)
  rw [pvd 129 126 132 123 (by norm_num)] at hfin
  refine ⟨hfin.1, ?_⟩
  have : (0x4e14a45e8ec305e233e11b4174e214ac : Nat) + 2 ^ 123 < 2 ^ 127 := by norm_num
  omega

theorem evTree_lt {x : Nat} (hv : vTree x < 2 ^ 126) : evTree x < 2 ^ 127 := (evTree_facts hv).2
theorem evTree_ge {x : Nat} (hv : vTree x < 2 ^ 126) :
    0x4e14a45e8ec305e233e11b4174e214ac ≤ evTree x := (evTree_facts hv).1

/-- Two-sided bound on the odd Horner accumulator: `0x270a… ≤ od < 2^126`. -/
theorem odTree_facts {x : Nat} (hv : vTree x < 2 ^ 126) :
    0x270a522f476182f119f08da0ba710a56 ≤ odTree x ∧ odTree x < 2 ^ 126 := by
  have hod : odTree x =
  ...
  rw [hod]
  set v := vTree x with hvdef
  have h0 : evmAdd 0xc926ddbf3830ca5561cc01585402d0 (evmShr 0x83 (evmMul 0xdc07aff85e5bb5629d0fb64a84bb v)) < 2 ^ 121 := by
  ...
  set od0 := evmAdd 0xc926ddbf3830ca5561cc01585402d0 (evmShr 0x83 (evmMul 0xdc07aff85e5bb5629d0fb64a84bb v)) with hod0
  have h1 : evmAdd 0xad4506b00b1246c7e5b4fd33e1201b (evmShr 0x89 (evmMul od0 v)) < 2 ^ 121 := by
    have := (stage_bounds (c := 0xad4506b00b1246c7e5b4fd33e1201b) (prev := od0) (v := v)
      (P := 2 ^ 121) (V := 2 ^ 126) (sh := 0x89) h0 hv (by norm_num) (by norm_num)
      (by rw [pvd 121 126 137 110 (by norm_num)]; norm_num)).2
    rw [pvd 121 126 137 110 (by norm_num)] at this; omega
  set od1 := evmAdd 0xad4506b00b1246c7e5b4fd33e1201b (evmShr 0x89 (evmMul od0 v)) with hod1
  have h2 : evmAdd 0xaf5662483c4ce783a9ef5fe025f42e9e (evmShr 0x7f (evmMul od1 v)) < 2 ^ 129 := by
    have := (stage_bounds (c := 0xaf5662483c4ce783a9ef5fe025f42e9e) (prev := od1) (v := v)
      (P := 2 ^ 121) (V := 2 ^ 126) (sh := 0x7f) h1 hv (by norm_num) (by norm_num)
      (by rw [pvd 121 126 127 120 (by norm_num)]; norm_num)).2
    rw [pvd 121 126 127 120 (by norm_num)] at this; omega
  set od2 := evmAdd 0xaf5662483c4ce783a9ef5fe025f42e9e (evmShr 0x7f (evmMul od1 v)) with hod2
  have hfin := stage_bounds (c := 0x270a522f476182f119f08da0ba710a56) (prev := od2) (v := v)
    (P := 2 ^ 129) (V := 2 ^ 126) (sh := 0x87) h2 hv (by norm_num) (by norm_num)
    (by rw [pvd 129 126 135 120 (by norm_num)]; norm_num)
  rw [pvd 129 126 135 120 (by norm_num)] at hfin
  refine ⟨hfin.1, ?_⟩
  have : (0x270a522f476182f119f08da0ba710a56 : Nat) + 2 ^ 119 < 2 ^ 126 := by norm_num
  omega

theorem odTree_lt {x : Nat} (hv : vTree x < 2 ^ 126) : odTree x < 2 ^ 126 := (odTree_facts hv).2
theorem odTree_ge {x : Nat} (hv : vTree x < 2 ^ 126) :
    0x270a522f476182f119f08da0ba710a56 ≤ odTree x := (odTree_facts hv).1
