...
`r1Tree x = shr(67 − k, r0 − MARGIN)` closes the kernel: the quotient already carries the
...
* **range**: `r0 < 2^129` keeps the shift argument below `2^129`, and the at-least-two-bit shift
  floors it below `2^127 < 2^254`.
-/
...
theorem closing_shift {x : Nat} (hx : x < 2 ^ 256)
  ...
  -- 67 (as int) - int256 k, transported through evmSub
  have h67 : int256 (0x43 : Nat) = 67 := by
  ...
  have hsub : int256 (evmSub 0x43 (kTree x)) = 67 - int256 (kTree x) := by
    have := evmSub_transport (a := 0x43) (b := kTree x) (by norm_num) hkw
      (by rw [h67, hip255]; omega)
      (by rw [h67, hip255]; omega)
    rw [h67] at this; exact this
...
`[2^124, 2^129)`: its signed value is in `[2^124 − MARGIN, 2^129)`, in particular nonnegative
and below `2^129`. -/
theorem shiftArg_bounds_of {r0 : Nat} (hr0w : r0 < 2 ^ 256)
    (hr0_lo : (2 ^ 124 : Int) ≤ int256 r0) (hr0_hi : int256 r0 < 2 ^ 129) :
    int256 (evmSub r0 0x1) = int256 r0 - 0x1 ∧
      0 ≤ int256 r0 - 0x1 ∧
      int256 r0 - 0x1 < 2 ^ 129 := by
...
/-- Abstract closing-shift facts over an opaque shift argument word `W` and shift `s ∈ [2, 128]`
with `int256 W ∈ [0, 2^129)`: the floor `shr(s, W)` is nonnegative and below `2^127`. -/
...
/-- The closing-shift result fits in the nonnegative half of `int128`. -/
theorem r1Tree_int128_range {x : Nat} (hx : x < 2 ^ 256)
    (hC : int256 Cmask < int256 x) (hC0 : int256 x < int256 C0thresh) :
    r1Tree x < 2 ^ 127 := by
  obtain ⟨s, hseq, hslo, hshi, _⟩ := closing_shift hx hC hC0
  obtain ⟨hr0lo, hr0hi⟩ := r0Tree_bounds hx hC hC0
  ...
  have hcast : ((r1Tree x : Nat) : Int) < 2 ^ 127 := by
    rw [← hi]
    exact hlt
...
theorem r1Tree_range {x : Nat} (hx : x < 2 ^ 256)
    (hC : int256 Cmask < int256 x) (hC0 : int256 x < int256 C0thresh) :
    r1Tree x < 2 ^ 254 :=
  lt_trans (r1Tree_int128_range hx hC hC0) (by norm_num)
