...
theorem domain_of_below_C0 {x : Nat} (hx : x < 2 ^ 256) (h : int256 x < int256 C0thresh) :
    u256 x < 0x92b2f16cc66c5a4ae96e80d4 ∨ 2 ^ 255 ≤ u256 x := by
  rw [u256_id hx]
  ...
  · left
    ...
    rw [this] at h
    have : (x : Int) < 45401140326676417766828703956 := h
    have hC0 : (0x92b2f16cc66c5a4ae96e80d4 : Nat) = 45401140326676417766828703956 := by norm_num
...
analytic bound `SeamR0Bound` (`r0Tree x1 + 3 ≤ 2·r0Tree x2` across one octave). -/
