...
def evS1 (x : Nat) : Nat := evmAdd 0x9a036222841f47c6ed6fc3f7599445 (evmShr 0x95 (evmMul (evS0 x) (vTree x)))
def evS2 (x : Nat) : Nat := evmAdd 0x9064d9657e9a21fc16bb69331b81ae1e (evmShr 0x7b (evmMul (evS1 x) (vTree x)))
...
theorem evS1_lt {x : Nat} (hv : vTree x < 2 ^ 120) : evS1 x < 2 ^ 121 := by
  have := (stage_bounds (c := 0x9a036222841f47c6ed6fc3f7599445) (prev := evS0 x) (v := vTree x)
...
theorem evS2_lt {x : Nat} (hv : vTree x < 2 ^ 120) : evS2 x < 2 ^ 129 := by
  have := (stage_bounds (c := 0x9064d9657e9a21fc16bb69331b81ae1e) (prev := evS1 x) (v := vTree x)
...
theorem evS3_lt {x : Nat} (hv : vTree x < 2 ^ 120) : evS3 x < 2 ^ 129 := by
  have := (stage_bounds (c := 0x93f11e650dd6c64b96ce79065cdf80f4) (prev := evS2 x) (v := vTree x)
...
def odS0 (x : Nat) : Nat := evmAdd 0xc926ddbecdeeb42e68cd16db7ed378 (evmShr 0x7e (evmMul 0xdc07aff8276bde9a361278df6a10 (vTree x)))
def odS1 (x : Nat) : Nat := evmAdd 0xad4506af99be27419341e181693281 (evmShr 0x84 (evmMul (odS0 x) (vTree x)))
def odS2 (x : Nat) : Nat := evmAdd 0xaf566247c05753b42892f77b67a6b7c7 (evmShr 0x7a (evmMul (odS1 x) (vTree x)))

theorem odTree_layers (x : Nat) :
    odTree x = evmAdd 0x9c2948bcaca16a0dd2fe98bb4470c388 (evmShr 0x80 (evmMul (odS2 x) (vTree x))) :=
  rfl
...
theorem odS1_lt {x : Nat} (hv : vTree x < 2 ^ 120) : odS1 x < 2 ^ 121 := by
  have := (stage_bounds (c := 0xad4506af99be27419341e181693281) (prev := odS0 x) (v := vTree x)
...
theorem odS2_lt {x : Nat} (hv : vTree x < 2 ^ 120) : odS2 x < 2 ^ 129 := by
  have := (stage_bounds (c := 0xaf566247c05753b42892f77b67a6b7c7) (prev := odS1 x) (v := vTree x)
...
theorem evTree_lip {x1 x2 : Nat} (hv1 : vTree x1 < 2 ^ 120) (hv2 : vTree x2 < 2 ^ 120)
  ...
  have d1 : dist_le (evS1 x1) (evS1 x2) 941485 := by
    have h := stage_lip_dist (c := 0x9a036222841f47c6ed6fc3f7599445)
  ...
  have d2 : dist_le (evS2 x1) (evS2 x2) 2658573678 := by
    have h := stage_lip_dist (c := 0x9064d9657e9a21fc16bb69331b81ae1e) (P := 2 ^ 121) (V := 2 ^ 120)
      (sh := 0x7b) (W := Wstep)
  ...
  have d3 : dist_le (evS3 x1) (evS3 x2) 10639016494 := by
    have h := stage_lip_dist (c := 0x93f11e650dd6c64b96ce79065cdf80f4) (P := 2 ^ 129) (V := 2 ^ 120)
      (sh := 0x81) (W := Wstep)
...
theorem odTree_lip {x1 x2 : Nat} (hv1 : vTree x1 < 2 ^ 120) (hv2 : vTree x2 < 2 ^ 120)
  ...
  have d0 : dist_le (odS0 x1) (odS0 x2) 649038 := by
    have h := stage_lip_dist (c := 0xc926ddbecdeeb42e68cd16db7ed378) (P := 2 ^ 112) (V := 2 ^ 120)
      (sh := 0x7e) (W := Wstep)
  ...
  have d1 : dist_le (odS1 x1) (odS1 x2) 5192456 := by
    have h := stage_lip_dist (c := 0xad4506af99be27419341e181693281) (P := 2 ^ 121) (V := 2 ^ 120)
      (sh := 0x84) (W := Wstep)
  ...
  have d2 : dist_le (odS2 x1) (odS2 x2) 5318210098 := by
    have h := stage_lip_dist (c := 0xaf566247c05753b42892f77b67a6b7c7) (P := 2 ^ 121) (V := 2 ^ 120)
      (sh := 0x7a) (W := Wstep)
