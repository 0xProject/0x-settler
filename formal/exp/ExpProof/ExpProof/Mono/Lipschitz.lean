...
hence move `v = ⌊t²/2^133⌋` by at most `W = ⌊(G + 1)/2^5⌋ + 1`. The even/odd accumulators
`Ev`/`Od` are then nearly constant: each Horner stage `evmAdd c (evmShr sh (evmMul prev v))`
changes by a bounded amount under a bounded change of `v` (and of the incoming accumulator
`prev`), and the five/four stages compose to the constants `DEv`/`DOd`.
...
/-- One Horner stage's Lipschitz bound. With `prev_i ≤ P`, `v_i < V`, the products fitting a
word, `|v2 − v1| ≤ W` and `|prev2 − prev1| ≤ Dprev`, the stage output `evmAdd c (evmShr sh (evmMul
prev v))` moves by at most `⌊(P·W + V·Dprev)/2^sh⌋ + 1`. Stated as a two-sided `Nat` distance
over opaque words. The argument cap `V` is a parameter because the monic-fed stage has no
power-of-two headroom: its `P·V` fits a word only against the exact coefficient literal. -/
theorem stage_lip {c prev1 prev2 v1 v2 P V Dprev W sh : Nat}
    ...
    evmAdd c (evmShr sh (evmMul prev1 v1)) ≤
        evmAdd c (evmShr sh (evmMul prev2 v2)) + ((P * W + V * Dprev) / 2 ^ sh + 1) ∧
      evmAdd c (evmShr sh (evmMul prev2 v2)) ≤
        evmAdd c (evmShr sh (evmMul prev1 v1)) + ((P * W + V * Dprev) / 2 ^ sh + 1) := by
  -- the caps fit a word: `V` by hypothesis, `P` because `P ≤ P·V < 2^256` (`V ≥ 1` from `v1 < V`)
  have hV1 : 0 < V := by omega
  have hP256 : P < 2 ^ 256 := by
    have := Nat.le_mul_of_pos_right P hV1
    omega
  -- products are exact (fit a word) and the stage adds are exact (no overflow)
  have hpvbnd : ∀ p v : Nat, p ≤ P → v < V → p * v < 2 ^ 256 ∧ p * v ≤ P * V := by
    intro p v hp hv
    have h1 : p * v ≤ P * v := Nat.mul_le_mul_right _ hp
    have h2 : P * v ≤ P * V := Nat.mul_le_mul_left _ (le_of_lt hv)
  ...
  have hsh1 : prev1 * v1 / 2 ^ sh ≤ P * V / 2 ^ sh := Nat.div_le_div_right hpv1le
  have hsh2 : prev2 * v2 / 2 ^ sh ≤ P * V / 2 ^ sh := Nat.div_le_div_right hpv2le
  ...
  rw [he1, he2]
  -- bound the product difference: |prev2·v2 − prev1·v1| ≤ P·W + V·Dprev
  have hprodbound : ∀ pa pb va vb : Nat, pa ≤ P → pb ≤ P → va < V → vb < V →
      va ≤ vb + W → pa ≤ pb + Dprev →
      pa * va ≤ pb * vb + (P * W + V * Dprev) := by
    intro pa pb va vb hpa hpb hva hvb hvgap hpgap
    -- pa·va ≤ pa·(vb + W) = pa·vb + pa·W ≤ pa·vb + P·W
    -- pa·vb ≤ (pb + Dprev)·vb = pb·vb + Dprev·vb ≤ pb·vb + Dprev·V
    ...
    have t6 : Dprev * vb ≤ Dprev * V := Nat.mul_le_mul_left _ (le_of_lt hvb)
    have t7 : Dprev * V = V * Dprev := Nat.mul_comm _ _
    omega
