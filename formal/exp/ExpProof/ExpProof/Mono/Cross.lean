...
/-- The `mul scaleQ67 N` dividend as a plain `Nat` product when `N`'s signed value is in
`[0, 2^128)`: `evmMul scaleQ67 N = scaleQ67 * N` (no wrap), and `N` is its own signed value. -/
theorem mulScale_transport {N : Nat} (hNw : N < 2 ^ 256) (hNnn : 0 ≤ int256 N)
    (hNlt : int256 N < 2 ^ 129) :
    evmMul scaleQ67 N = scaleQ67 * N ∧ N < 2 ^ 129 := by
  ...
  have hfit : scaleQ67 * N < 2 ^ 256 := by
    have h1 : scaleQ67 * N ≤ scaleQ67 * 2 ^ 129 := Nat.mul_le_mul_left _ (le_of_lt hNnat)
...
theorem r0_mono_of_cross {E1 TD1 E2 TD2 : Nat}
    ...
    int256 (evmDiv (evmMul scaleQ67 (evmAdd E1 TD1)) (evmSub E1 TD1)) ≤
      int256 (evmDiv (evmMul scaleQ67 (evmAdd E2 TD2)) (evmSub E2 TD2)) := by
  ...
  have hsw : scaleQ67 < 2 ^ 256 := by unfold scaleQ67; norm_num
  have hfit1 : scaleQ67 * evmAdd E1 TD1 < 2 ^ 256 := by
    have h1 : scaleQ67 * evmAdd E1 TD1 ≤ scaleQ67 * 2 ^ 129 :=
      Nat.mul_le_mul_left _ (le_of_lt hN1nat)
    have h2 : scaleQ67 * 2 ^ 129 < 2 ^ 256 := by unfold scaleQ67; norm_num
    omega
  have hfit2 : scaleQ67 * evmAdd E2 TD2 < 2 ^ 256 := by
    have h1 : scaleQ67 * evmAdd E2 TD2 ≤ scaleQ67 * 2 ^ 129 :=
      Nat.mul_le_mul_left _ (le_of_lt hN2nat)
    have h2 : scaleQ67 * 2 ^ 129 < 2 ^ 256 := by unfold scaleQ67; norm_num
    omega
  -- the two quotients as plain Nat floor divisions
  have hq1 : evmDiv (evmMul scaleQ67 (evmAdd E1 TD1)) (evmSub E1 TD1) =
      scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 := by
    rw [hA1, evmDiv_eq hfit1 (evmSub_lt _ _) hD1nz]
  have hq2 : evmDiv (evmMul scaleQ67 (evmAdd E2 TD2)) (evmSub E2 TD2) =
      scaleQ67 * evmAdd E2 TD2 / evmSub E2 TD2 := by
  ...
  have hqle : scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 ≤
      scaleQ67 * evmAdd E2 TD2 / evmSub E2 TD2 := by
    rw [Nat.le_div_iff_mul_le hD2pos']
    have hfl : scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 * evmSub E1 TD1 ≤
        scaleQ67 * evmAdd E1 TD1 := Nat.div_mul_le_self _ _
    -- (q1·D2)·D1 ≤ S·N1·D2 ≤ S·N2·D1 ⇒ q1·D2 ≤ S·N2 (divide by D1 > 0)
    have hstep : scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 * evmSub E2 TD2 * evmSub E1 TD1 ≤
        scaleQ67 * evmAdd E2 TD2 * evmSub E1 TD1 := by
      calc scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 * evmSub E2 TD2 * evmSub E1 TD1
          = scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 * evmSub E1 TD1 * evmSub E2 TD2 := by ring
        _ ≤ scaleQ67 * evmAdd E1 TD1 * evmSub E2 TD2 := Nat.mul_le_mul_right _ hfl
        _ = scaleQ67 * (evmAdd E1 TD1 * evmSub E2 TD2) := by ring
        _ ≤ scaleQ67 * (evmAdd E2 TD2 * evmSub E1 TD1) := Nat.mul_le_mul_left _ hcrossN
        _ = scaleQ67 * evmAdd E2 TD2 * evmSub E1 TD1 := by ring
  ...
  have hD2ge : 2 ^ 126 ≤ evmSub E2 TD2 := by
    ...
    exact_mod_cast h
  have hq1small : scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 < 2 ^ 255 := by
    have h1 : scaleQ67 * evmAdd E1 TD1 / evmSub E1 TD1 ≤ scaleQ67 * evmAdd E1 TD1 / 2 ^ 126 :=
      Nat.div_le_div_left hD1ge (Nat.two_pow_pos _)
    have h2 : scaleQ67 * evmAdd E1 TD1 / 2 ^ 126 < 2 ^ 130 := by
      rw [Nat.div_lt_iff_lt_mul (Nat.two_pow_pos _)]
      calc scaleQ67 * evmAdd E1 TD1 < 2 ^ 256 := hfit1
        _ = 2 ^ 130 * 2 ^ 126 := by norm_num
    have h3 : (2:Nat) ^ 130 < 2 ^ 255 := by norm_num
    omega
  have hq2small : scaleQ67 * evmAdd E2 TD2 / evmSub E2 TD2 < 2 ^ 255 := by
    have h1 : scaleQ67 * evmAdd E2 TD2 / evmSub E2 TD2 ≤ scaleQ67 * evmAdd E2 TD2 / 2 ^ 126 :=
      Nat.div_le_div_left hD2ge (Nat.two_pow_pos _)
    have h2 : scaleQ67 * evmAdd E2 TD2 / 2 ^ 126 < 2 ^ 130 := by
      rw [Nat.div_lt_iff_lt_mul (Nat.two_pow_pos _)]
