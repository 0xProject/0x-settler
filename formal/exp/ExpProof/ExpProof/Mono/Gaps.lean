...
/-- `K27 = G·2^107 + r` with `0 < r < 2^107`: the reduced-argument constant's quotient by the
shift is exactly `G`. -/
theorem K27_decomp :
    (0x279d346de4781f921dd7a89933d54d1f72928 : Int) =
      Gstep * 2 ^ 107 + 152274402897802763547896603683112 := by
  unfold Gstep; norm_num

theorem Gstep_rem_pos : (0 : Int) < 152274402897802763547896603683112 := by norm_num
theorem Gstep_rem_lt : (152274402897802763547896603683112 : Int) < 2 ^ 107 := by norm_num
...
theorem tTree_step {x1 x2 : Nat} (hx1 : x1 < 2 ^ 256) (hx2 : x2 < 2 ^ 256)
  ...
  have hp107 : (0 : Int) < 2 ^ 107 := by norm_num
  ...
  -- combine: 2^107·t1 ≤ A < 2^107·t1 + 2^107 and 2^107·t2 ≤ A + K27 < 2^107·t2 + 2^107
  set A := K27 * X1 - LN2 * k
