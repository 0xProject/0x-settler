...
theorem evalYUB (t : Int) : evalPoly yUB t = (2 ^ 130 + 1) * evalPoly numExpV t := by
  unfold yUB; rw [evalPoly_polyScale]

theorem evalWUB (t : Int) : evalPoly wUB t = 2 ^ 130 * evalPoly denExpV t := by
  unfold wUB; rw [evalPoly_polyScale]

theorem evalYLB (t : Int) : evalPoly yLB t = (2 ^ 130 - 1) * evalPoly numExpV t := by
  unfold yLB; rw [evalPoly_polyScale]

theorem evalWLB (t : Int) : evalPoly wLB t = 2 ^ 130 * evalPoly denExpV t := by
...
theorem capExpUp {t : Int} (h1 : 0 ≤ t) (h2 : t ≤ (H128 : Int)) :
  ...
  have hc120 : (0 : Int) ≤ 2 ^ 130 + 1 := by norm_num
  have hp120 : (0 : Int) ≤ 2 ^ 130 := by norm_num
...
theorem capExpLo {t : Int} (h1 : 0 ≤ t) (h2 : t ≤ (H128 : Int)) :
  ...
  have hden : 1 ≤ evalPoly denExpV t := denExpV_ge_one h1 h2
  have hden0 : 0 ≤ evalPoly denExpV t := by omega
  have hc126 : (0 : Int) ≤ 2 ^ 130 - 1 := by norm_num
  have hp126 : (0 : Int) ≤ 2 ^ 130 := by norm_num
