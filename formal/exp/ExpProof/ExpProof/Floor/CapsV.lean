...
theorem evalYUB (t : Int) : evalPoly yUB t = (2 ^ 120 + 1) * evalPoly numExpV t := by
  unfold yUB; rw [evalPoly_polyScale]

theorem evalWUB (t : Int) : evalPoly wUB t = 2 ^ 120 * evalPoly denExpV t := by
  unfold wUB; rw [evalPoly_polyScale]

theorem evalYLB (t : Int) : evalPoly yLB t = (2 ^ 126 - 1) * evalPoly numExpV t := by
  unfold yLB; rw [evalPoly_polyScale]

theorem evalWLB (t : Int) : evalPoly wLB t = 2 ^ 126 * evalPoly denExpV t := by
