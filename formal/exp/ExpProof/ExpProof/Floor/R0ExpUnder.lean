...
/-- **Link-1 under (nonneg half)**: `1000·(2¹²⁶·NUMv − r0·DENv) ≤ 6001·DENv`. The floor residual
costs one denominator; the odd-truncation carry `(2⁶³⁸ + Wod·2⁴⁸⁰·t)·(2¹²⁶ + r0)` fits in five
(`t ≤ H128`, `r0 ≤ 1.45·2¹²⁶`, `den ≥ 0.72·2¹²⁶`). -/
theorem link1_under_int {x : Nat} (hx : x < 2 ^ 256)
    ...
    1000 * (2 ^ 126 * NUMv (vTree x) (int256 (tTree x)) -
      ...
      6001 * DENv (vTree x) (int256 (tTree x)) := by
  ...
  have hLHS : 2 ^ 126 * NUMv (vTree x) (int256 (tTree x)) -
      int256 (r0Tree x) * DENv (vTree x) (int256 (tTree x)) ≤
      2 ^ 638 * ((evTree x : Int) - int256 (todTree x)) +
        (2 ^ 638 + 1075052609 * 2 ^ 480 * int256 (tTree x)) * (2 ^ 126 + int256 (r0Tree x)) := by
    unfold NUMv DENv
    set r0 := int256 (r0Tree x) with hr0def
    set ev := (evTree x : Int) with hevdef
    set tod := int256 (todTree x) with htoddef
    ...
    -- Ep·2^110·(2^126−r0) ≤ 2^638·ev·(2^126−r0)
    have hterm1 : Ep * 2 ^ 110 * (2 ^ 126 - r0) ≤ 2 ^ 638 * ev * (2 ^ 126 - r0) := by
      apply mul_le_mul_of_nonpos_right _ h2126r0_np
      nlinarith [hEp_lo]
    -- t·Op·(2^126+r0) ≤ (2^638·tod + 2^638 + Wod·2^480·t)·(2^126+r0)
    have hterm2 : t * Op * (2 ^ 126 + r0) ≤
        (2 ^ 638 * tod + 2 ^ 638 + 1075052609 * 2 ^ 480 * t) * (2 ^ 126 + r0) :=
      mul_le_mul_of_nonneg_right htOp_hi hr0p_nn
    -- floor: 2^126·num − r0·den < den, scaled by 2^638
    have hfloor : (2:Int) ^ 126 * (ev + tod) - r0 * (ev - tod) ≤ (ev - tod) := by
      linarith [hfloor_hi]
    have hfloor638 : (2:Int) ^ 638 * ((2:Int) ^ 126 * (ev + tod) - r0 * (ev - tod)) ≤
        2 ^ 638 * (ev - tod) := mul_le_mul_of_nonneg_left hfloor (by positivity)
  ...
  have hA : 2 ^ 638 * den ≤ D + 2 * 2 ^ 638 := by rw [hDdef]; linarith [hDEN_ge]
  have hDlow : (2:Int) ^ 638 * (61251667532612381706986956632087880162 - 2) ≤ D := by
    have h1 : (2:Int) ^ 638 * (61251667532612381706986956632087880162 - 2) ≤
        2 ^ 638 * den - 2 * 2 ^ 638 := by nlinarith [hden]
    rw [hDdef]; linarith [h1, hDEN_ge]
  have hB : (2 ^ 638 + 1075052609 * 2 ^ 480 * t) * (2 ^ 126 + r0) ≤ 5 * D := by
    have hcoef : 2 ^ 638 + 1075052609 * 2 ^ 480 * t ≤
        2 ^ 638 + 1075052609 * 2 ^ 480 * 117932881612756647068972071382077242199 := by
      have := mul_le_mul_of_nonneg_left hthi (by positivity : (0:Int) ≤ 1075052609 * 2 ^ 480)
      linarith [this]
    have hr0p_nn : (0:Int) ≤ 2 ^ 126 + r0 := by linarith [(r0_bracket_nonneg hx hC hC0 htnn).1]
    have h1 : (2 ^ 638 + 1075052609 * 2 ^ 480 * t) * (2 ^ 126 + r0) ≤
        (2 ^ 638 + 1075052609 * 2 ^ 480 * 117932881612756647068972071382077242199) *
          (2 ^ 126 + r0) := mul_le_mul_of_nonneg_right hcoef hr0p_nn
    have h2 : 100 * ((2 ^ 638 + 1075052609 * 2 ^ 480 * 117932881612756647068972071382077242199) *
        (2 ^ 126 + r0)) ≤
        (2 ^ 638 + 1075052609 * 2 ^ 480 * 117932881612756647068972071382077242199) *
          (245 * 2 ^ 126) := by
      have hr0cap : 100 * (2 ^ 126 + r0) ≤ 245 * 2 ^ 126 := by linarith [hr0hi145]
      nlinarith [hr0cap]
    have h3 : (2 ^ 638 + 1075052609 * 2 ^ 480 * 117932881612756647068972071382077242199) *
        (245 * 2 ^ 126) ≤ 500 * (2 ^ 638 * (61251667532612381706986956632087880162 - 2)) := by
      norm_num
    have h4 : (500 : Int) * (2 ^ 638 * (61251667532612381706986956632087880162 - 2)) ≤ 500 * D :=
      mul_le_mul_of_nonneg_left hDlow (by norm_num)
    linarith [h1, h2, h3, h4]
  have hC2000 : (2000 : Int) * 2 ^ 638 ≤ D := by
    have : (2000 : Int) * 2 ^ 638 ≤ 2 ^ 638 * (61251667532612381706986956632087880162 - 2) := by
      norm_num
    linarith [this, hDlow]
  linarith [hLHS, hA, hB, hC2000]

/-- **Link-1 under (nonpositive half)**: same `6001/1000` budget; the even-truncation width and the
`tod`-floor unit are absorbed by `DENv ≥ 2⁶³⁸·ev ≥ 2⁶³⁸·A0`. -/
theorem link1_under_int_neg {x : Nat} (hx : x < 2 ^ 256)
  ...
  obtain ⟨htod_lo125, _, _, _⟩ := todTree_bound hx hC hC0
  have hLHS : 2 ^ 126 * NUMv (vTree x) (int256 (tTree x)) -
      int256 (r0Tree x) * DENv (vTree x) (int256 (tTree x)) ≤
      2 ^ 638 * ((evTree x : Int) - int256 (todTree x)) +
        283678831804417 * 2 ^ 590 * 2 ^ 126 + 2 * 2 ^ 638 * 2 ^ 126 := by
    unfold NUMv DENv
    set r0 := int256 (r0Tree x) with hr0def
    set ev := (evTree x : Int) with hevdef
    set tod := int256 (todTree x) with htoddef
    ...
    -- Ep·2^110·(2^126−r0) ≤ 2^638·ev·(2^126−r0) + Wev·2^590·2^126
    have hterm1 : Ep * 2 ^ 110 * (2 ^ 126 - r0) ≤
        2 ^ 638 * ev * (2 ^ 126 - r0) + 283678831804417 * 2 ^ 590 * 2 ^ 126 := by
      have h1 : Ep * 2 ^ 110 * (2 ^ 126 - r0) ≤
          (2 ^ 638 * ev + 283678831804417 * 2 ^ 590) * (2 ^ 126 - r0) := by
        apply mul_le_mul_of_nonneg_right _ h2126r0_nn
        nlinarith [hEp_hi]
      have h2 : (283678831804417 : Int) * 2 ^ 590 * (2 ^ 126 - r0) ≤
          283678831804417 * 2 ^ 590 * 2 ^ 126 :=
        mul_le_mul_of_nonneg_left h2126r0_le (by positivity)
      nlinarith [h1, h2]
    -- t·Op·(2^126+r0) ≤ (2^638·tod + 2^638)·(2^126+r0) ≤ 2^638·tod·(2^126+r0) + 2·2^638·2^126
    have hterm2 : t * Op * (2 ^ 126 + r0) ≤
        2 ^ 638 * tod * (2 ^ 126 + r0) + 2 * 2 ^ 638 * 2 ^ 126 := by
      have h1 : t * Op * (2 ^ 126 + r0) ≤ (2 ^ 638 * tod + 2 ^ 638) * (2 ^ 126 + r0) :=
        mul_le_mul_of_nonneg_right htOp_hi hr0p_nn
      have h2 : (2:Int) ^ 638 * (2 ^ 126 + r0) ≤ 2 ^ 638 * (2 * 2 ^ 126) :=
    ...
    have hfloor638 : (2:Int) ^ 638 * ((2:Int) ^ 126 * (ev + tod) - r0 * (ev - tod)) ≤
        2 ^ 638 * (ev - tod) := mul_le_mul_of_nonneg_left hfloor (by positivity)
    nlinarith [hterm1, hterm2, hfloor638]
  -- budget against DENv ≥ 2^638·ev ≥ 2^638·A0; den ≤ ev + 2^125
  set ev := (evTree x : Int) with hevdef
  set tod := int256 (todTree x) with htoddef
  set D := DENv (vTree x) (int256 (tTree x)) with hDdef
  have hev : (103786963397729689639908782561058906594 : Int) ≤ ev := by
    have : (0x4e14a45e5650b506e97f4c5da23861e2 : Int) ≤ ev := by
  ...
  have hden_le : ev - tod ≤ ev + 2 ^ 125 := by
    ...
    linarith [this]
  have hDev : 2 ^ 638 * ev ≤ D := hDEN_ge
  -- 1000·(2^638·(ev + 2^125) + Wev·2^590·2^126 + 2·2^638·2^126) ≤ 1000·2^638·ev + 5001·2^638·A0
  have hlit : 1000 * (2 ^ 638 * 2 ^ 125 + 283678831804417 * 2 ^ 590 * 2 ^ 126 +
      2 * 2 ^ 638 * 2 ^ 126) ≤
      (5001 : Int) * (2 ^ 638 * 103786963397729689639908782561058906594) := by
    norm_num
  have hAev : (5001 : Int) * (2 ^ 638 * 103786963397729689639908782561058906594) ≤ 5001 * D := by
    have h1 : (2:Int) ^ 638 * 103786963397729689639908782561058906594 ≤ 2 ^ 638 * ev :=
...
/-- **The per-point deficit (nonneg half).** `2¹²⁶·exp(rt) ≤ r0 + 67/10`: link-1 `≤ 6001/1000`, the
`Mp` factor `≤ 1/20`, the under gap `≤ 37/100`; the granularity is free on this half. -/
theorem r0_real_under_tight {x : Nat} (hx : x < 2 ^ 256)
    (hC : int256 Cmask < int256 x) (hC0 : int256 x < int256 C0thresh)
    (htnn : 0 ≤ int256 (tTree x)) :
    (2 ^ 126 : Real) * Real.exp (reducedArg x) ≤ (int256 (r0Tree x) : Real) + 67 / 10 := by
  obtain ⟨_, hthi⟩ := tTree_in_cert_domain hx hC hC0
  ...
  have hr0R : (r0 : Real) ≤ (145 / 100) * (2 ^ 126 : Real) := by
    ...
    push_cast at h
    linarith [h]
  have hEt_bound : (2 ^ 126 : Real) * Et ≤ (r0 : Real) + 6001 / 1000 + 1 / 20 := by
    ...
    have h3 : ((2 ^ 126 : Real) * ((NUMv v t : Real) / (DENv v t : Real))) * (Mpp - 1) ≤ 1 / 20 := by
      ...
      have hcap : (2 ^ 126 : Real) * ((NUMv v t : Real) / (DENv v t : Real)) ≤
          (145 / 100) * (2 ^ 126 : Real) + 6001 / 1000 := by linarith [hQv_le, hr0R]
      have := mul_le_mul_of_nonneg_right hcap (by positivity : (0:Real) ≤ 1 / (2 ^ 131 : Real))
      have hfin : ((145 / 100) * (2 ^ 126 : Real) + 6001 / 1000) * (1 / (2 ^ 131 : Real)) ≤
          1 / 20 := by norm_num
      linarith [this, hfin]
  ...
  have hsum : (6001 : Real) / 1000 + 1 / 20 + 37 / 100 ≤ 67 / 10 := by norm_num
...
theorem r0_real_under_tight_neg {x : Nat} (hx : x < 2 ^ 256)
  ...
  have hEt_bound : (2 ^ 126 : Real) * Et ≤ (r0 : Real) + 6001 / 1000 + 1 / 20 +
    ...
    have hMpterm : ((2 ^ 126 : Real) * ((NUMv v t : Real) / (DENv v t : Real))) * (Mp - 1) ≤
      ...
      rw [hMp1]
      have hcap : (2 ^ 126 : Real) * ((NUMv v t : Real) / (DENv v t : Real)) ≤
          (2 ^ 126 : Real) + 6001 / 1000 := by linarith [hQv_le, hr0R]
      have := mul_le_mul_of_nonneg_right hcap
        (by positivity : (0:Real) ≤ 1 / ((2 ^ 131 : Real) - 1))
      have hfin : ((2 ^ 126 : Real) + 6001 / 1000) * (1 / ((2 ^ 131 : Real) - 1)) ≤ 1 / 20 := by
...
/-- **Per-point deficit (tight, any sign):** `2¹²⁶·exp(rt) ≤ r0 + 67/10`. -/
...
(`r0 ≥ 2¹²⁶·exp(rt) − 67/10 > 2¹²⁶·(1/2) − 67/10 > 2¹²⁴`.) -/
theorem r0Tree_gt_2_124 {x : Nat} (hx : x < 2 ^ 256)
  ...
  have h3 : (2 : Real) ^ 124 + 67 / 10 < (2 ^ 125 : Real) := by norm_num
...
theorem r0_seam_double {x1 x2 : Nat}
  ...
  -- 2^126·E1 = 2·(2^126·E2)·y ≤ 2·(r0_2 + 67/10)·y
  ...
  have hkey : 2 * ((int256 (r0Tree x2) : Real) + 67 / 10) * y +
    ...
    have hexpand : 2 * ((int256 (r0Tree x2) : Real) + 67 / 10) * (1 - 1 / (2 * (10 ^ 27 : Real))) =
        2 * (int256 (r0Tree x2) : Real) + 67 / 5 -
          ((int256 (r0Tree x2) : Real) + 67 / 10) / (10 ^ 27 : Real) := by field_simp; ring
    have hbig : ((int256 (r0Tree x2) : Real) + 67 / 10) / (10 ^ 27 : Real) > 1562 := by
