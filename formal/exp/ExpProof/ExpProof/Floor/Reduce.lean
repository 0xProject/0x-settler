...
The sum is below `2/2¹²⁹`, and each bound holds over the whole wide region.
-/
...
/-- **Reduced-argument tight over bound (gap-1, one-sided).** On the wide region the integer
...
theorem reducedArg_close_over_wide {x : Nat} (hx : x < 2 ^ 256) (hW : WideRegion x) :
  ...
  -- bound P1 : |P1| < 2^97·N/(2^235·10^27) where N = K27·10^27 − 2^235 = 222636907558699806209605632
  -- We bound P1 ∈ (−ε, ε) with ε = 2^97·N/(2^235·10^27) < 2⁻¹³⁹.  Use explicit endpoints.
  have hXloR : -(158456325028528675187087900672 : Real) < XR := by
    have := (@Int.cast_lt Real _ _ _ _ _ _ _).mpr hxlo; rw [hXRdef]
    rw [show ((2:Int)^97 : Int) = 158456325028528675187087900672 from by norm_num] at this
    push_cast at this; linarith [this]
  have hXhiR : XR < (158456325028528675187087900672 : Real) := by
  ...
  -- |P1| < 2⁻¹³⁹ (a generous bound):  |XR| < 2^97, |coeff| = m, and 2^97·m < 2⁻¹³⁹.
  have hP1_abs : |P1| < 1 / (64 * N128) := by
    ...
    rw [hco_abs]
    have hX_abs : |XR| < 158456325028528675187087900672 := abs_lt.mpr ⟨hXloR, hXhiR⟩
    ...
    calc |XR| * (222636907558699806209605632 / (N235 * (10 ^ 27 : Real)))
        < 158456325028528675187087900672 * (222636907558699806209605632 / (N235 * (10 ^ 27 : Real))) :=
          (mul_lt_mul_right hco_pos).mpr hX_abs
      _ = 158456325028528675187087900672 * 222636907558699806209605632 /
...
theorem reducedArg_close_over {x : Nat} (hx : x < 2 ^ 256)
    ...
    (int256 (tTree x) : Real) / (2 ^ 129 : Real) - reducedArg x < 1 / (32 * (2 ^ 129 : Real)) :=
  reducedArg_close_over_wide hx (wideRegion_of_wad hC hC0)
...
theorem reducedArg_close_under_wide {x : Nat} (hx : x < 2 ^ 256) (hW : WideRegion x) :
  ...
  have hP2_abs : |P2| < 1 / (2048 * N128) := by
    ...
    have hbound : |kR * (LN2R / N235 - LR)| ≤ 127 * (1 / N235) := by
      rw [abs_mul]
      exact mul_le_mul h1 h2 (abs_nonneg _) (by norm_num)
    have hlt : 127 * (1 / N235) < 1 / (2048 * N128) := by
      rw [hN235, hN128, mul_one_div, div_lt_div_iff₀ (by positivity) (by positivity)]; norm_num
    linarith [hbound, hlt]
  have hP3int_hi : 55213970774324510299478046898216203619608872 * X -
      38271408169742254668347313025622401492114385419650052359639581444463709 * k - 2 ^ 106 * t < 2 ^ 106 := by omega
...
theorem reducedArg_close_under {x : Nat} (hx : x < 2 ^ 256)
    (hC : int256 Cmask < int256 x) (hC0 : int256 x < int256 C0thresh) :
    reducedArg x - (int256 (tTree x) : Real) / (2 ^ 129 : Real) < 1025 / (1024 * (2 ^ 129 : Real)) :=
  reducedArg_close_under_wide hx (wideRegion_of_wad hC hC0)
...
/-- **Reduced-argument real bound (gap-1).** On the wide region the reduced argument `rt`
...
theorem reducedArg_close_wide {x : Nat} (hx : x < 2 ^ 256) (hW : WideRegion x) :
    |reducedArg x - (int256 (tTree x) : Real) / (2 ^ 129 : Real)| < 9 / (8 * (2 ^ 129 : Real)) := by
  have hover := reducedArg_close_over_wide hx hW
  have hunder := reducedArg_close_under_wide hx hW
  set d : Real := reducedArg x - (int256 (tTree x) : Real) / (2 ^ 129 : Real) with hd
  have hp128 : (0 : Real) < (2 ^ 129 : Real) := by positivity
  rw [abs_lt]
  constructor
  · -- −d < 1/(32·2¹²⁹) ≤ 9/(8·2¹²⁹)
    have h32 : (1 : Real) / (32 * (2 ^ 129 : Real)) ≤ 9 / (8 * (2 ^ 129 : Real)) := by
      rw [div_le_div_iff₀ (by positivity) (by positivity)]; nlinarith [hp128]
    have : -d < 1 / (32 * (2 ^ 129 : Real)) := by rw [hd]; linarith [hover]
    linarith [this, h32]
  · -- d < 1025/(1024·2¹²⁹) ≤ 9/(8·2¹²⁹)
    have h1025 : (1025 : Real) / (1024 * (2 ^ 129 : Real)) ≤ 9 / (8 * (2 ^ 129 : Real)) := by
      rw [div_le_div_iff₀ (by positivity) (by positivity)]; nlinarith [hp128]
    have : d < 1025 / (1024 * (2 ^ 129 : Real)) := by rw [hd]; exact hunder
    linarith [this, h1025]

theorem reducedArg_close {x : Nat} (hx : x < 2 ^ 256)
    (hC : int256 Cmask < int256 x) (hC0 : int256 x < int256 C0thresh) :
    |reducedArg x - (int256 (tTree x) : Real) / (2 ^ 129 : Real)| < 9 / (8 * (2 ^ 129 : Real)) :=
  reducedArg_close_wide hx (wideRegion_of_wad hC hC0)
