...
The real pre-floor accumulator is `accumReal x = (r0 − MARGIN) / 2^(68 − k)` on the `2⁶⁸` output
...
(`closing_shift`: the shift word is `68 − int256 k`, nonnegative), the accumulator takes the
closed form `((int256 r0) − MARGIN) / 2^s` with `s = 68 − int256 (kTree x)`, the form the
