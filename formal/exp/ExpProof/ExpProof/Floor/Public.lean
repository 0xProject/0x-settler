...
theorem run_exp_ray_to_wad_evm_floorOrOneLess (H' : RuntimeAccumBound) (x : Nat) (hx : x < 2 ^ 256)
    ...
    ∃ r, run_exp_ray_to_wad_evm x = .ok r ∧ FloorOrOneLessBracket x (int256 r) := by
  ...
  · by_cases hz : x = 0
    · -- scale point
      ...
      rw [h0]; exact floorOrOneLess_zero
  ...
  · -- below/at the clamp boundary: result is 0, E < 2
    ...
    refine ⟨?_, ?_⟩
    · rw [Int.cast_zero]
      have hpos : (0 : Real) ≤ expRayToWadTarget x := by
        unfold expRayToWadTarget
        have := Real.exp_pos ((x : Real) / (RAY : Real))
        positivity
      exact hpos
    · have := H'.belowC x hC
...
theorem run_exp_ray_to_wad_evm_underByAtMostOne (H' : RuntimeAccumBound) (x : Nat) (hx : x < 2 ^ 256)
    (hC0 : int256 x < int256 C0thresh) :
    ∃ r, run_exp_ray_to_wad_evm x = .ok r ∧ UnderByAtMostOne x (int256 r) := by
...
theorem run_exp_ray_to_wad_evm_exactFloor (H' : RuntimeAccumBound) (x : Nat) (hx : x < 2 ^ 256)
    ...
    ∃ r, run_exp_ray_to_wad_evm x = .ok r ∧ ExactFloorBracket x (int256 r) := by
