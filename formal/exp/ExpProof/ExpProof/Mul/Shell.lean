...
theorem mulScaleTree_spec {y : Nat} (hy : y < 2 ^ 256) (habs : absTree y ≤ scaleQ67) :
    scaleShiftTree (absTree y) < 256 ∧
      mulScaleTree y = absTree y * 2 ^ scaleShiftTree (absTree y) ∧
      mulScaleTree y ≤ scaleQ67 := by
  ...
  · have hclz : evmClz 0 = 256 := by
    ...
    have hsst : scaleShiftTree (absTree y) = 127 := by
      ...
      simp only [h0, hclz, hs0, hshl, hgt, hsub]
    have hscale : mulScaleTree y = 0 := by
      unfold mulScaleTree
      rw [hsst, h0, hshl]
    rw [hsst, hscale, h0]
    exact ⟨by norm_num, by ring, by unfold scaleQ67; norm_num⟩
  · have hlog : Nat.log2 (absTree y) ≤ 126 := by
      have h1 : 2 ^ Nat.log2 (absTree y) ≤ absTree y := Nat.log2_self_le (by omega)
      ...
      omega
    have hlt : absTree y < 2 ^ (Nat.log2 (absTree y) + 1) := Nat.lt_log2_self
    have hclz : evmClz (absTree y) = 255 - Nat.log2 (absTree y) := by
      unfold evmClz
      rw [u256_self haylt, if_neg (by omega)]
    have hs0eq : evmSub (evmClz (absTree y)) scaleMaxClz = 126 - Nat.log2 (absTree y) := by
    ...
    have hfit : absTree y * 2 ^ (126 - Nat.log2 (absTree y)) < 2 ^ 127 := by
      calc absTree y * 2 ^ (126 - Nat.log2 (absTree y))
          < 2 ^ (Nat.log2 (absTree y) + 1) * 2 ^ (126 - Nat.log2 (absTree y)) :=
            mul_lt_mul_of_pos_right hlt (Nat.two_pow_pos _)
        _ = 2 ^ (Nat.log2 (absTree y) + 1 + (126 - Nat.log2 (absTree y))) := by
            rw [← pow_add]
    ...
    by_cases hover : absTree y * 2 ^ (126 - Nat.log2 (absTree y)) > scaleQ67
    · have hgt : evmGt (absTree y * 2 ^ (126 - Nat.log2 (absTree y))) scaleQ67 = 1 := by
        unfold evmGt
        rw [u256_self (lt_trans hfit (by norm_num)),
          u256_self (by unfold scaleQ67; norm_num), if_pos hover]
      have hs0pos : 0 < 126 - Nat.log2 (absTree y) := by
        rcases Nat.eq_zero_or_pos (126 - Nat.log2 (absTree y)) with h | h
        · rw [h] at hover
          simp at hover
          omega
        · exact h
      have hsub : evmSub (126 - Nat.log2 (absTree y)) 1 = 126 - Nat.log2 (absTree y) - 1 :=
        evmSub_small (by omega) (by omega)
      have hsst : scaleShiftTree (absTree y) = 126 - Nat.log2 (absTree y) - 1 := by
        unfold scaleShiftTree
        simp only [hs0eq, hshl, hgt, hsub]
      have hfit' : absTree y * 2 ^ (126 - Nat.log2 (absTree y) - 1) < 2 ^ 126 := by
        calc absTree y * 2 ^ (126 - Nat.log2 (absTree y) - 1)
            < 2 ^ (Nat.log2 (absTree y) + 1) * 2 ^ (126 - Nat.log2 (absTree y) - 1) :=
              mul_lt_mul_of_pos_right hlt (Nat.two_pow_pos _)
          _ = 2 ^ (Nat.log2 (absTree y) + 1 + (126 - Nat.log2 (absTree y) - 1)) := by
              rw [← pow_add]
          _ ≤ 2 ^ 126 := Nat.pow_le_pow_right (by norm_num) (by omega)
      have hshl' : evmShl (126 - Nat.log2 (absTree y) - 1) (absTree y) =
          absTree y * 2 ^ (126 - Nat.log2 (absTree y) - 1) :=
        evmShl_small (by omega) haylt
          (lt_trans hfit' (by norm_num : (2 : Nat) ^ 126 < 2 ^ 256))
      have hscale : mulScaleTree y = absTree y * 2 ^ (126 - Nat.log2 (absTree y) - 1) := by
        unfold mulScaleTree
        rw [hsst, hshl']
      rw [hsst, hscale]
      have h126 : (2 : Nat) ^ 126 ≤ scaleQ67 := by unfold scaleQ67; norm_num
      exact ⟨by omega, rfl, by omega⟩
    · have hgt : evmGt (absTree y * 2 ^ (126 - Nat.log2 (absTree y))) scaleQ67 = 0 := by
        unfold evmGt
        rw [u256_self (lt_trans hfit (by norm_num)),
          u256_self (by unfold scaleQ67; norm_num), if_neg hover]
      have hsub : evmSub (126 - Nat.log2 (absTree y)) 0 = 126 - Nat.log2 (absTree y) :=
        evmSub_small (by omega) (by omega)
      have hsst : scaleShiftTree (absTree y) = 126 - Nat.log2 (absTree y) := by
        unfold scaleShiftTree
        simp only [hs0eq, hshl, hgt, hsub]
      have hscale : mulScaleTree y = absTree y * 2 ^ (126 - Nat.log2 (absTree y)) := by
        unfold mulScaleTree
        rw [hsst, hshl]
      rw [hsst, hscale]
      exact ⟨by omega, rfl, by omega⟩
...
private theorem r0MulTree_scale_point {y : Nat} (hy : y < 2 ^ 256)
    (habs : absTree y ≤ scaleQ67) : r0MulTree y 0 = mulScaleTree y := by
  ...
  exact evmDiv_exact (by unfold ev4; norm_num) (mulScaleTree_lt y) (by unfold ev4; norm_num)
    (by
      calc mulScaleTree y * ev4 ≤ scaleQ67 * ev4 := Nat.mul_le_mul_right _ hcap
        _ < 2 ^ 256 := by unfold scaleQ67 ev4; norm_num)
...
theorem mulMagnitudeTree_scale_point {y : Nat} (hy : y < 2 ^ 256)
    (hpos : 0 < absTree y) (habs : absTree y ≤ scaleQ67) :
    mulMagnitudeTree y 0 = absTree y := by
  obtain ⟨hs256, hscale, hcap⟩ := mulScaleTree_spec hy habs
  have hQlt : scaleQ67 < 2 ^ 256 := by unfold scaleQ67; norm_num
  ...
  have hshr : evmShr (scaleShiftTree (absTree y))
    ...
    rw [evmShr_small hs256 (by omega)]
...
theorem mulExpTree_scale_point {y : Nat} (hy : y < 2 ^ 256) (habs : absTree y ≤ scaleQ67) :
    mulExpTree y 0 = y := by
...
/-- **Scale point.** `mulExpRay(y, 0)` returns `y` for every supported magnitude. -/
theorem run_mul_exp_ray_evm_scale_point {y : Nat} (hy : y < 2 ^ 256)
  ...
  have hguard : mulExpGuardTree y 0 = 0 := by
    ...
    refine ⟨habs, ?_, Or.inl int256_zero_word⟩
    rw [int256_mulExpRayHi, int256_zero_word]
    norm_num
  have h := run_mul_exp_ray_evm_eq_tree_of_guard y 0 hguard
  rwa [mulExpTree_scale_point hy habs] at h

/-- **Clamp.** `mulExpRay(y, x)` returns zero at or below the zero cutoff, for every supported
magnitude. -/
theorem run_mul_exp_ray_evm_clamped {y x : Nat} (hy : y < 2 ^ 256) (hx : x < 2 ^ 256)
    (habs : absTree y ≤ scaleQ67) (hclamp : int256 x ≤ int256 mulExpRayZeroMax) :
    run_mul_exp_ray_evm y x = .ok 0 := by
  have hguard : mulExpGuardTree y x = 0 := by
    rw [mulExpGuardTree_eq_zero_iff hx]
    refine ⟨habs, ?_, Or.inr (Or.inl hclamp)⟩
    rw [int256_mulExpRayHi]
    rw [int256_mulExpRayZeroMax] at hclamp
    omega
  have h := run_mul_exp_ray_evm_eq_tree_of_guard y x hguard
...
theorem mulExpRay_run_bracket_scale_point {y : Nat} (hy : y < 2 ^ 256)
    (habs : absTree y ≤ scaleQ67) : MulExpRayRunBracket y 0 := by
  refine ⟨y, run_mul_exp_ray_evm_scale_point hy habs, ?_⟩
  rw [int256_zero_word]
...
theorem clamped_target_lt_one {y x : Nat} (hy : y < 2 ^ 256) (hx : x < 2 ^ 256)
    (habs : absTree y ≤ scaleQ67) (hclamp : int256 x ≤ int256 mulExpRayZeroMax) :
  ...
  have hpow : (2 : ℝ) ^ (127 : ℕ) * Real.exp z = Real.exp (z + 127 * Real.log 2) := by
    ...
    ring
  have hnat : ((int256 y).natAbs : ℝ) ≤ ((scaleQ67 : Nat) : ℝ) := by
    have h := absTree_eq_natAbs hy
    exact_mod_cast (h ▸ habs)
  have hQ : ((scaleQ67 : Nat) : ℝ) < (2 : ℝ) ^ (127 : ℕ) := by
    unfold scaleQ67
    norm_num
...
theorem mulExpRay_run_bracket_clamped {y x : Nat} (hy : y < 2 ^ 256) (hx : x < 2 ^ 256)
    (habs : absTree y ≤ scaleQ67) (hclamp : int256 x ≤ int256 mulExpRayZeroMax) :
    MulExpRayRunBracket y x := by
  refine ⟨0, run_mul_exp_ray_evm_clamped hy hx habs hclamp, ?_⟩
  rw [int256_zero_word]
  have hlt := clamped_target_lt_one hy hx habs hclamp
