...
multiplier takes the same guard: the unconditional upper fence at the first octave past the deficit
envelope and the accuracy test on the closing shift. Each predicate mirrors one comparison of the
compiled guard; `int256 (mulShiftTree y x) < 2` is exactly the runtime's `slt(shift, 2)`.
-/
...
theorem int128Word_max : Int128Word int128Max := by
  unfold Int128Word
  decide
...
def MulExpRayPanicDomain (y x : Nat) : Prop :=
  MulExpRayCanonical y x ∧
    (int256 mulExpRayHi ≤ int256 x ∨ int256 (mulShiftTree y x) < 2)
...
theorem mulExpRay_value_or_panic_of_canonical {y x : Nat} (hcanon : MulExpRayCanonical y x) :
  ...
  · by_cases hshift : 2 ≤ int256 (mulShiftTree y x)
    · exact Or.inl ⟨hcanon, hxhi, hshift⟩
    · exact Or.inr ⟨hcanon, Or.inr (by omega)⟩
...
theorem mulExpRay_value_not_panic {y x : Nat} :
    MulExpRayValueDomain y x → ¬ MulExpRayPanicDomain y x := by
  intro hv hp
  obtain ⟨_, hxhi, hlive⟩ := hv
  obtain ⟨_, hbadHi | hbadShift⟩ := hp
  · omega
  · omega
...
theorem mulExpGuardTree_eq_ite {y x : Nat} (hx : x < 2 ^ 256) :
    mulExpGuardTree y x =
      if int256 mulExpRayHi ≤ int256 x ∨ int256 (mulShiftTree y x) < 2 then 1 else 0 := by
...
theorem mulExpGuardTree_eq_zero_iff {y x : Nat} (hx : x < 2 ^ 256) :
    mulExpGuardTree y x = 0 ↔
      int256 x < int256 mulExpRayHi ∧ 2 ≤ int256 (mulShiftTree y x) := by
...
theorem mulExpGuardTree_eq_one_iff {y x : Nat} (hx : x < 2 ^ 256) :
    mulExpGuardTree y x = 1 ↔
      int256 mulExpRayHi ≤ int256 x ∨ int256 (mulShiftTree y x) < 2 := by
