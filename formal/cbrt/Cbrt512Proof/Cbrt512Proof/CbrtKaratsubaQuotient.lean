...
-- ============================================================================
-- Karatsuba quotient correctness (both quotient and remainder)
-- ============================================================================

/-- Both components of the Karatsuba quotient: .1 = floor((res * 2^86 + limb_hi) / d),
    .2 = (res * 2^86 + limb_hi) % d. -/
private theorem kq_both_correct
    (res limb_hi d : Nat)
  ...
  · -- === NO-CARRY CASE: res / 2^170 = 0 ===
    ...
    exact ⟨by rw [hn_eq, evmDiv_eq' _ d (by rw [← hn_eq]; exact hn_evm_lt) hd_pos hd_wm],
           by rw [hn_eq, evmMod_eq' _ d hn_lt hd_pos hd_wm]⟩
...
theorem model_cbrtKaratsubaQuotient_evm_correct
    ...
    (model_cbrtKaratsubaQuotient_evm res limb_hi d).2 =
      (res * 2 ^ 86 + limb_hi) % d :=
  kq_both_correct res limb_hi d hres hlimb hd_ge hd_bound hres_bound hlimb_bound
