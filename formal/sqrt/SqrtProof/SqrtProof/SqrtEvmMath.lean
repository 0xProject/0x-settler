...
namespace SqrtEvmMath

open FormalYul
open SqrtCertified
open SqrtCert

theorem normStep_eq_bstep (x z : Nat) :
    normShr 1 (normAdd z (normDiv x z)) = bstep x z := by
  simp [normShr, normAdd, normDiv, bstep]

theorem normSeed_eq_sqrtSeed_of_pos (x : Nat) (hx : 0 < x) (hx256 : x < 2 ^ 256) :
...
theorem word_mod_gt_256 : 256 < WORD_MOD := by
  unfold WORD_MOD
  decide

theorem u256_eq_of_lt (x : Nat) (hx : x < WORD_MOD) : u256 x = x :=
  u256_eq_self_of_lt hx

theorem evmClz_eq_normClz_of_u256 (x : Nat) (hx : x < WORD_MOD) :
    evmClz x = normClz x := by
  unfold evmClz normClz
  simp [u256_eq_of_lt x hx]

theorem normClz_le_256 (x : Nat) : normClz x <= 256 := by
  unfold normClz
  split <;> omega

theorem evmSub_eq_normSub_of_le
...
theorem evmDiv_eq_normDiv_of_u256
...
theorem evmAdd_eq_normAdd_of_no_overflow
...
theorem evmLt_eq_normLt_of_u256
    (a b : Nat) (ha : a < WORD_MOD) (hb : b < WORD_MOD) :
    evmLt a b = normLt a b := by
  unfold evmLt normLt
  simp [u256_eq_of_lt a ha, u256_eq_of_lt b hb]

theorem evmGt_eq_normGt_of_u256
    (a b : Nat) (ha : a < WORD_MOD) (hb : b < WORD_MOD) :
    evmGt a b = normGt a b := by
  unfold evmGt normGt
  simp [u256_eq_of_lt a ha, u256_eq_of_lt b hb]

theorem evmShr_eq_normShr_of_u256
...
theorem evmShl_eq_normShl_of_safe
...
theorem two_pow_lt_word (n : Nat) (hn : n < 256) :
    2 ^ n < WORD_MOD := by
  unfold WORD_MOD
  exact Nat.pow_lt_pow_right (by decide : 1 < (2 : Nat)) hn

theorem zero_lt_word : (0 : Nat) < WORD_MOD := by
  unfold WORD_MOD
  decide

theorem one_lt_word : (1 : Nat) < WORD_MOD := by
  unfold WORD_MOD
  decide

theorem pow128_plus_one_lt_word : 2 ^ 128 + 1 < WORD_MOD := by
  unfold WORD_MOD
  decide
...
theorem sqrtLog_norm_lt_256 (x : Nat) :
...
theorem sqrtLog_evm_eq_norm (x : Nat) (hx : x < WORD_MOD) :
...
theorem seed_evm_eq_norm (x : Nat) (hx : x < WORD_MOD) :
...
theorem step_evm_eq_norm_of_safe
...
theorem m_lt_pow128_of_u256 (m x : Nat) (hmlo : m * m <= x) (hx : x < WORD_MOD) :
    m < 2 ^ 128 := by
...
theorem innerSqrt_lt_word (x : Nat) (hx : x < WORD_MOD) : innerSqrt x < WORD_MOD := by
...
theorem x_div_m_le_m_plus_two (x m : Nat) (hm : 0 < m)
...
theorem sum_lt_word_of_cert (x m z d : Nat) (hx : x < WORD_MOD) (hm : 0 < m)
...
theorem seed_sum_lt_word (i : Fin 256) (x : Nat)
...
theorem floor_correction_norm_eq_if (x z : Nat) :
...
theorem floor_step_evm_eq_norm
    (x z : Nat)
    (hx : x < WORD_MOD)
    (hz : z < WORD_MOD) :
...
theorem floorSqrt_evmCorrection_eq
    (x : Nat)
    (hx : x < WORD_MOD) :
...
theorem floorSqrt_lt_word (x : Nat) (hx : x < WORD_MOD) :
...
theorem sqrtUp256_lt_word (x : Nat) (hx : x < WORD_MOD) :
...
theorem sqrtUp_step_evm_eq_inner_round
    (x z : Nat)
    (hxW : x < WORD_MOD)
    (hzLe128 : z <= 2 ^ 128) :
...
theorem innerSqrt_evmSteps_eq (x : Nat) (hx256 : x < WORD_MOD) :
