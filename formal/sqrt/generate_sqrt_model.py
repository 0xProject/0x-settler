#!/usr/bin/env python3
"""
Generate Lean models of Sqrt.sol directly from Solidity source.

This script extracts `_sqrt`, `sqrt`, and `sqrtUp` from `src/vendor/Sqrt.sol` and
...
DEFAULT_FUNCTION_ORDER = ("_sqrt", "sqrt", "sqrtUp")
...
def build_model_body(assignments: tuple[Assignment, ...], *, evm: bool) -> str:
    ...
    if evm:
        ...
        call_map = {
            "_sqrt": "model_sqrt_evm",
            "sqrt": "model_sqrt_floor_evm",
            "sqrtUp": "model_sqrt_up_evm",
        }
...
def parse_function_selection(args: argparse.Namespace) -> tuple[str, ...]:
    ...
    # sqrt/sqrtUp depend on _sqrt.
    if ("sqrt" in selected or "sqrtUp" in selected) and "_sqrt" not in selected:
        selected.append("_sqrt")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Lean model of Sqrt.sol functions from Solidity source"
    )
    parser.add_argument(
        "--solidity",
        default="src/vendor/Sqrt.sol",
        help="Path to Solidity source file containing Sqrt library",
    )
    parser.add_argument(
        "--functions",
        default="",
        help="Comma-separated function names to model (default: _sqrt,sqrt,sqrtUp)",
    )
    parser.add_argument(
        "--function",
        action="append",
