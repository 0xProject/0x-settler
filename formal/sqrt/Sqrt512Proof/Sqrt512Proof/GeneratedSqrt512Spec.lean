/-
  ...
  Proves that 6 Babylonian steps from the fixed seed
  240615969168004511545033772477625056927 ≈ floor(sqrt(2^255))
...
/-- Error bound for octave 254: run6Fixed x - m ≤ fd6_254. -/
private theorem run6Fixed_error_254
  ...
  have hmz2 : m ≤ z2 := babylon_step_floor_bound x z1 m hz1Pos hmlo
  have hz2Pos : 0 < z2 := Nat.lt_of_lt_of_le hm hmz2
  have hmz3 : m ≤ z3 := babylon_step_floor_bound x z2 m hz2Pos hmlo
  have hz3Pos : 0 < z3 := Nat.lt_of_lt_of_le hm hmz3
  have hmz4 : m ≤ z4 := babylon_step_floor_bound x z3 m hz3Pos hmlo
  have hz4Pos : 0 < z4 := Nat.lt_of_lt_of_le hm hmz4
  have hmz5 : m ≤ z5 := babylon_step_floor_bound x z4 m hz4Pos hmlo
  ...
  have hd2 : z2 - m ≤ fd2_254 := by
    have h := step_from_bound x m lo254 z1 fd1_254 hm lo254_pos hlo hmhi hmz1 hd1 hd1m
    simpa [z2, fd2_254] using h
  have hd2m : fd2_254 ≤ m := Nat.le_trans fd2_254_le_lo hlo

  have hd3 : z3 - m ≤ fd3_254 := by
    have h := step_from_bound x m lo254 z2 fd2_254 hm lo254_pos hlo hmhi hmz2 hd2 hd2m
    simpa [z3, fd3_254] using h
  have hd3m : fd3_254 ≤ m := Nat.le_trans fd3_254_le_lo hlo

  have hd4 : z4 - m ≤ fd4_254 := by
    have h := step_from_bound x m lo254 z3 fd3_254 hm lo254_pos hlo hmhi hmz3 hd3 hd3m
    simpa [z4, fd4_254] using h
  have hd4m : fd4_254 ≤ m := Nat.le_trans fd4_254_le_lo hlo

  have hd5 : z5 - m ≤ fd5_254 := by
    have h := step_from_bound x m lo254 z4 fd4_254 hm lo254_pos hlo hmhi hmz4 hd4 hd4m
    simpa [z5, fd5_254] using h
  have hd5m : fd5_254 ≤ m := Nat.le_trans fd5_254_le_lo hlo

  have hd6 : z6 - m ≤ fd6_254 := by
    have h := step_from_bound x m lo254 z5 fd5_254 hm lo254_pos hlo hmhi hmz5 hd5 hd5m
    simpa [z6, fd6_254] using h
...
private def maxAbs255 : Nat :=
  max (FIXED_SEED - lo255) (hi255 - FIXED_SEED)

private def fd1_255 : Nat :=
  (maxAbs255 * maxAbs255 + 2 * hi255) / (2 * FIXED_SEED)
...
/-- Error bound for octave 255: run6Fixed x - m ≤ fd6_255. -/
private theorem run6Fixed_error_255
  ...
  have hd2 : z2 - m ≤ fd2_255 := by
    have h := step_from_bound x m lo255 z1 fd1_255 hm lo255_pos hlo hmhi hmz1 hd1 hd1m
    simpa [z2, fd2_255] using h
  have hd2m : fd2_255 ≤ m := Nat.le_trans fd2_255_le_lo hlo

  have hd3 : z3 - m ≤ fd3_255 := by
    have h := step_from_bound x m lo255 z2 fd2_255 hm lo255_pos hlo hmhi hmz2 hd2 hd2m
    simpa [z3, fd3_255] using h
  have hd3m : fd3_255 ≤ m := Nat.le_trans fd3_255_le_lo hlo

  have hd4 : z4 - m ≤ fd4_255 := by
    have h := step_from_bound x m lo255 z3 fd3_255 hm lo255_pos hlo hmhi hmz3 hd3 hd3m
    simpa [z4, fd4_255] using h
  have hd4m : fd4_255 ≤ m := Nat.le_trans fd4_255_le_lo hlo

  have hd5 : z5 - m ≤ fd5_255 := by
    have h := step_from_bound x m lo255 z4 fd4_255 hm lo255_pos hlo hmhi hmz4 hd4 hd4m
    simpa [z5, fd5_255] using h
  have hd5m : fd5_255 ≤ m := Nat.le_trans fd5_255_le_lo hlo

  have hd6 : z6 - m ≤ fd6_255 := by
    have h := step_from_bound x m lo255 z5 fd5_255 hm lo255_pos hlo hmhi hmz5 hd5 hd5m
    simpa [z6, fd6_255] using h

  simpa [run6Fixed, z1, z2, z3, z4, z5, z6] using hd6

-- ============================================================================
-- Section 4: Combined fixed-seed bracket
-- ============================================================================

/-- Lower bound: m ≤ run6Fixed x for any m with m² ≤ x and x > 0. -/
private theorem m_le_run6Fixed (x m : Nat) (hx : 0 < x) (hmlo : m * m ≤ x) :
  ...
  have hm_z6 : m ≤ z6 := babylon_step_floor_bound x z5 m hz5 hmlo
  simpa [run6Fixed, z1, z2, z3, z4, z5, z6] using hm_z6

/-- 1-ULP bracket: natSqrt x ≤ run6Fixed x ≤ natSqrt x + 1 for x ∈ [2^254, 2^256). -/
theorem fixed_seed_bracket (x : Nat)
  ...
  have hm : 0 < natSqrt x := by
    ...
    intro h0
    have := natSqrt_lt_succ_sq x
    rw [h0] at this
    omega
  constructor
  · exact m_le_run6Fixed x (natSqrt x) hx hmlo
  · suffices run6Fixed x - natSqrt x ≤ 1 by omega
    by_cases hlt : x < 2 ^ 255
    · -- Octave 254
      have hOct : 2 ^ (254 : Fin 256).val ≤ x ∧ x < 2 ^ ((254 : Fin 256).val + 1) :=
        ⟨hlo, hlt⟩
      have hint := m_within_cert_interval ⟨254, by omega⟩ x (natSqrt x) hmlo hmhi hOct
      exact Nat.le_trans
        (run6Fixed_error_254 x (natSqrt x) hm hmlo hmhi hint.1 hint.2) fd6_254_le_one
    · -- Octave 255
      have h255 : 2 ^ 255 ≤ x := Nat.le_of_not_lt hlt
      have hOct : 2 ^ (⟨255, by omega⟩ : Fin 256).val ≤ x ∧
          x < 2 ^ ((⟨255, by omega⟩ : Fin 256).val + 1) :=
        ⟨h255, hhi⟩
      have hint := m_within_cert_interval ⟨255, by omega⟩ x (natSqrt x) hmlo hmhi hOct
      exact Nat.le_trans
        (run6Fixed_error_255 x (natSqrt x) hm hmlo hmhi hint.1 hint.2) fd6_255_le_one

-- ============================================================================
-- Section 5: Floor correction
-- ============================================================================

/-- x / z < z ↔ x < z * z for z > 0. -/
private theorem div_lt_iff_sq (x z : Nat) (hz : 0 < z) :
    x / z < z ↔ x < z * z :=
  Nat.div_lt_iff_lt_mul hz

/-- Floor correction with the fixed seed gives natSqrt for x ∈ [2^254, 2^256). -/
theorem floorSqrt_fixed_eq_natSqrt (x : Nat)
    (hlo : 2 ^ 254 ≤ x) (hhi : x < 2 ^ 256) :
  ...
  have h1 : floorSqrt_fixed x =
    ...
    dsimp only []
    exact if_neg (Nat.ne_of_gt hz_pos)
  rw [h1]
  simp only [show (x / run6Fixed x < run6Fixed x) = (x < run6Fixed x * run6Fixed x) from
    propext (div_lt_iff_sq x (run6Fixed x) hz_pos)]
  exact hcorr
