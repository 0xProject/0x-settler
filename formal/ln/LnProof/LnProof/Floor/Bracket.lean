...
is compared with the certificate bracket rationals: on `m ≥ S`,
`geTN2b/geTD2b ≤ X1/2^99`; on `m ≤ S`, the brackets trap
`-X1/2^99` between `ltTN2b/ltTD2b` and `ltTN/ltTD`. The chains run through
the exact division brackets of `z` and `u`, the Stages sandwiches for the
Horner stages, and divided-difference monotonicity of the homogenized `p`/`q`
polynomials.
-/
