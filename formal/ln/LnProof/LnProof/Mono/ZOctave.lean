...
theorem zWord_transport {m : Nat} (h1 : MLO ≤ m) (h2 : m < MHI) :
    int256 (evmShl 100 (evmSub Sc m)) =
        ((Sc : Int) - (m : Int)) * 1267650600228229401496703205376 ∧
      int256 (evmAdd m Sc) = (m : Int) + (Sc : Int) := by
  simp only [MLO, MHI] at h1 h2
  have hSc : int256 Sc = (Sc : Int) := toInt_of_lt (by simp only [Sc]; omega)
  have hm : int256 m = (m : Int) := toInt_of_lt (by simp only [Sc] at *; omega)
  have e1 : int256 (evmSub Sc m) = (Sc : Int) - (m : Int) := by
    rw [← hSc, ← hm]
    refine evmSub_transport (by simp only [Sc]; omega) (by omega) ?_ ?_ <;>
      rw [hSc, hm] <;> simp only [Sc, ipow255] <;> omega
  have e2 : int256 (evmShl 100 (evmSub Sc m)) =
  ...
  have e3 : int256 (evmAdd m Sc) = (m : Int) + (Sc : Int) := by
    rw [← hSc, ← hm]
...
theorem zWord_range {m : Nat} (h1 : MLO ≤ m) (h2 : m < MHI) :
    -(217494458298375249691265569570 : Int) ≤ int256 (zWord m) ∧
      int256 (zWord m) ≤ (217494458298375249691265569570 : Int) := by
  obtain ⟨e2, e3⟩ := zWord_transport h1 h2
  simp only [MLO, MHI] at h1 h2
  have hden : (0 : Int) < int256 (evmAdd m Sc) := by
    rw [e3]; simp only [Sc]; omega
  have hdenv : int256 (evmAdd m Sc) = (m : Int) + (Sc : Int) := e3
  unfold zWord
  rcases Int.le_total ((m : Int)) ((Sc : Int)) with hms | hms
  · -- numerator ≥ 0
    have hnum : (0 : Int) ≤ int256 (evmShl 100 (evmSub Sc m)) := by
    ...
    · have := Int.natCast_nonneg
        ((int256 (evmShl 100 (evmSub Sc m))).toNat / (int256 (evmAdd m Sc)).toNat)
      omega
    · have hq : (int256 (evmShl 100 (evmSub Sc m))).toNat /
          (int256 (evmAdd m Sc)).toNat < 217494458298375249691265569571 := by
      ...
      omega
  · -- numerator ≤ 0
    rcases Int.lt_or_le (int256 (evmShl 100 (evmSub Sc m))) 0 with hneg | hpos
    · rw [evmSdiv_neg_pos (evmShl_lt _ _) (evmAdd_lt _ _) hneg
      ...
      · have hq : (-int256 (evmShl 100 (evmSub Sc m))).toNat /
          ...
          rw [Nat.div_lt_iff_lt_mul (by rw [hdenv]; simp only [Sc]; omega)]
          rw [hdenv]
          have he2' : -int256 (evmShl 100 (evmSub Sc m)) =
...
theorem zWord_antitone_step {m : Nat} (h1 : MLO ≤ m) (h2 : m + 1 < MHI) :
    int256 (zWord (m + 1)) ≤ int256 (zWord m) := by
  ...
  simp only [MLO, MHI] at h1 h2
  have hden1 : (0 : Int) < int256 (evmAdd m Sc) := by rw [e3]; simp only [Sc]; omega
  have hden2 : (0 : Int) < int256 (evmAdd (m + 1) Sc) := by
    rw [f3]; simp only [Sc]; omega
  have hc1 : ((m + 1 : Nat) : Int) = (m : Int) + 1 := by omega
  have f2' : int256 (evmShl 100 (evmSub Sc (m + 1))) =
      ((Sc : Int) - (m : Int) - 1) * 1267650600228229401496703205376 := by
    rw [f2, hc1]
    rw [show ((Sc : Int) - ((m : Int) + 1)) = (Sc : Int) - (m : Int) - 1 by omega]
  have f3' : int256 (evmAdd (m + 1) Sc) = ((m : Int) + (Sc : Int)) + 1 := by
    rw [f3, hc1]
    omega
  ...
  · -- both numerators nonnegative
    have hn1 : (0 : Int) ≤ int256 (evmShl 100 (evmSub Sc m)) := by
      rw [e2]; exact Int.mul_nonneg (by omega) (by omega)
    have hn2 : (0 : Int) ≤ int256 (evmShl 100 (evmSub Sc (m + 1))) := by
    ...
    have hcross : int256 (evmShl 100 (evmSub Sc (m + 1))) * int256 (evmAdd m Sc) ≤
        int256 (evmShl 100 (evmSub Sc m)) * int256 (evmAdd (m + 1) Sc) := by
      rw [e2, e3, f2', f3']
    ...
    omega
  · -- P ≤ 0
    rcases Int.lt_or_le (int256 (evmShl 100 (evmSub Sc m))) 0 with hneg1 | hpos1
    · -- both numerators negative
      have hPneg : (Sc : Int) - (m : Int) ≤ 0 := by omega
      have hneg2 : int256 (evmShl 100 (evmSub Sc (m + 1))) < 0 := by
        rw [f2']
        have h := mul_le_mul_right_nonneg
          (show ((Sc : Int) - (m : Int) - 1) ≤ -1 by omega)
          (by omega : (0 : Int) ≤ 1267650600228229401496703205376)
        omega
      have hbnd1 : -(2 ^ 255) < int256 (evmShl 100 (evmSub Sc m)) := by
        rw [e2]
        have h := mul_le_mul_right_nonneg
        ...
        omega
      have hbnd2 : -(2 ^ 255) < int256 (evmShl 100 (evmSub Sc (m + 1))) := by
        rw [f2']
        have h := mul_le_mul_right_nonneg
      ...
      have hcross : (-int256 (evmShl 100 (evmSub Sc m))) * int256 (evmAdd (m + 1) Sc) ≤
          (-int256 (evmShl 100 (evmSub Sc (m + 1)))) * int256 (evmAdd m Sc) := by
        rw [e2, e3, f2', f3']
        ...
        rw [m1, m2]
        exact hz
      have hk1 : (0 : Int) ≤ -int256 (evmShl 100 (evmSub Sc m)) := by omega
      have hk2 : (0 : Int) ≤ -int256 (evmShl 100 (evmSub Sc (m + 1))) := by omega
    ...
    · -- P = 0: z(m) = 0, z(m+1) ≤ 0
      ...
      have h0 : int256 (evmShl 100 (evmSub Sc m)) = 0 := by
        rw [e2, hP0']
        omega
      rw [h0]
      simp only [Int.toNat_zero, Nat.zero_div]
      have := Int.natCast_nonneg
        ((-int256 (evmShl 100 (evmSub Sc (m + 1)))).toNat /
          (int256 (evmAdd (m + 1) Sc)).toNat)
      omega
...
theorem hAt_antitone {v w : Int}
    ...
    int256 (hAt w) ≤ int256 (hAt v) := by
  have key : ∀ n : Nat, v + (n : Int) ≤ (217494458298375249691265569570 : Int) →
      int256 (hAt (v + (n : Int))) ≤ int256 (hAt v) := by
    intro n
    induction n with
    | zero =>
      intro _
...
theorem zWord_antitone {m m' : Nat} (h1 : MLO ≤ m) (h2 : m ≤ m') (h3 : m' < MHI) :
    int256 (zWord m') ≤ int256 (zWord m) := by
  have key : ∀ n : Nat, m + n < MHI → int256 (zWord (m + n)) ≤ int256 (zWord m) := by
    intro n
    induction n with
...
theorem hAt_bound {w : Int}
    ...
    -(240000000000000000000000000000 : Int) ≤ int256 (hAt w) ∧
      int256 (hAt w) ≤ (240000000000000000000000000000 : Int) := by
  ...
  have hmw : evmMul (pS4 (uVal w)) (uint256OfInt w) < 2 ^ 256 := evmMul_lt _ _
  rw [heq]
  clear heq hul
  ...
  · -- numerator `P * w ≥ 0`
    have hnum : (0 : Int) ≤ int256 (evmMul pword (uint256OfInt w)) := by
      rw [hmul]; exact Int.mul_nonneg (by omega) hw
    rw [evmSdiv_pos_neg hmw qw hnum (by omega)]
    have s1 : int256 (evmMul pword (uint256OfInt w)) ≤
        int256 pword * 217494458298375249691265569570 := by
      rw [hmul]; exact mul_le_mul_left_nonneg h2 (by omega)
    have s2 : int256 pword * 217494458298375249691265569570 ≤
        13972178604861559108982341686387 * 217494458298375249691265569570 :=
      mul_le_mul_right_nonneg phi (by omega)
    have hdiv : (int256 (evmMul pword (uint256OfInt w))).toNat /
        (-int256 qword).toNat < 240000000000000000000000000001 := by
      rw [Nat.div_lt_iff_lt_mul (by omega)]
      omega
    have hge := Int.natCast_nonneg ((int256 (evmMul pword (uint256OfInt w))).toNat /
      (-int256 qword).toNat)
    omega
  · -- numerator `P * w ≤ 0`
    have hPnegw : (0 : Int) ≤ int256 pword * (-w) :=
      Int.mul_nonneg (by omega) (by omega)
    have hPe : int256 pword * (-w) = -(int256 pword * w) :=
      Int.mul_neg _ _
    rcases Int.lt_or_le (int256 (evmMul pword (uint256OfInt w))) 0 with hneg | hpos
    ...
    · rw [evmSdiv_pos_neg hmw qw hpos (by omega)]
      have hz : (int256 (evmMul pword (uint256OfInt w))).toNat = 0 := by omega
      rw [hz, Nat.zero_div]
      omega
...
theorem r1_mono {m m' : Nat} (h1 : MLO ≤ m) (h2 : m ≤ m') (h3 : m' < MHI) :
    int256 (x1W (zWord m)) ≤ int256 (x1W (zWord m')) := by
  ...
  have hwlt' : zWord m' < 2 ^ 256 := by unfold zWord; exact evmSdiv_lt _ _
  have e : x1W (zWord m) = hAt (int256 (zWord m)) := by
...
theorem r1_bound {m : Nat} (h1 : MLO ≤ m) (h2 : m < MHI) :
    -(240000000000000000000000000000 : Int) ≤ int256 (x1W (zWord m)) ∧
      int256 (x1W (zWord m)) ≤ (240000000000000000000000000000 : Int) := by
