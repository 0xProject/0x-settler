...
`lnTail one kw m` is the body pipeline downstream of the
`(eq, clz-exponent, mantissa)` triple. `lnWadToRayBody_eq_tail` re-expresses the
body decomposition through it, `ln2k_bound` brackets the `ln2 * k` term by
...
/-- `mul` and `add` are commutative on words. The generated Yul body writes their
...
theorem lnWadToRayBody_eq_tail {x : Nat} (_h : x < 2 ^ 256) :
    lnWadToRayBody x =
      lnTail (evmSub 160 (evmClz x)) (evmShr 160 (evmShl (evmClz x) x)) := by
  unfold lnWadToRayBody lnTail x1W pS4 pS3 pS2 pS1 qS5 qS4 qS3 qS2 qS1 uWord zWord
  ...
  rw [evmMul_comm 7450580596923828125,
    evmAdd_comm (evmMul 3273295013171879848905889459134067659407864468560
      (evmSub 160 (evmClz x))),
    evmAdd_comm 116873961749927929127912020551560854268589826112230]
