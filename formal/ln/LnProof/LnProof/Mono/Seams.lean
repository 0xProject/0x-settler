...
* within an octave, the mantissa-to-result map is nondecreasing -- proven in
  `LnProof.Mono.Step`/`LnProof.Mono.ZOctave`/`LnProof.Mono.Octave` from the
  polynomial certificates in `LnProof.Mono.Certs`;
...
`LnProof.Mono.Top` composes them into `lnWadToRayBody_mono`,
