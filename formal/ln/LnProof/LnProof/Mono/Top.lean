import LnProof.Mono.Octave
import LnProof.Mono.Seams
...
`lnWadToRayBody_mono` chains the unit step over the domain, and
`lnWadBody_mono` pushes the result through the wad helper's
floor division.
-/
...
theorem lnWadToRayBody_lt {x : Nat} (h : x < 2 ^ 256) : lnWadToRayBody x < 2 ^ 256 := by
...
theorem seam_extract {t : Nat} (ht1 : 1 ≤ t) (ht2 : t ≤ 254) :
  ...
  have h := lnWadToRayBody_seam_mono
...
theorem lnWadToRayBody_unit_step {x : Nat} (h1 : 1 ≤ x) (h2 : x + 1 < 2 ^ 255) :
  ...
  · -- x = 10^18 - 1: decided
    subst hsp
    have h := toInt_of_sle (lnWadToRayBody_lt (by omega)) (lnWadToRayBody_lt (by omega)) hd.1
  ...
  · rcases Decidable.em (x = 1000000000000000000) with hsp | hne2
    · -- x = 10^18: decided
      subst hsp
      have h := toInt_of_sle (lnWadToRayBody_lt (by omega)) (lnWadToRayBody_lt (by omega)) hd.2
      exact h
    · rcases Decidable.em (evmClz (x + 1) = evmClz x) with hclz | hclz
      · -- same octave: the analytic leg
        ...
        rw [lnWadToRayBody_eq_tail hx256, lnWadToRayBody_eq_tail hx1256, me, me', hclz]
      ...
      · -- clz seam: x + 1 is a power of two; decided
        ...
        have h := toInt_of_sle (lnWadToRayBody_lt (by omega)) (lnWadToRayBody_lt (by omega)) hs
...
theorem lnWadToRayBody_mono {x y : Nat} (hx : 0 < x) (hxy : x ≤ y)
  ...
  have key : ∀ n : Nat, x + n < 2 ^ 255 →
    ...
    | succ k ih =>
      intro hk
      have hs := lnWadToRayBody_unit_step (x := x + k) (by omega) (by omega)
      have he : x + (k + 1) = x + k + 1 := by omega
      rw [he]
      exact Int.le_trans (ih (by omega)) hs
  have hkey := key (y - x) (by omega)
  rw [show x + (y - x) = y by omega] at hkey
  exact sle_of_toInt (lnWadToRayBody_lt (by omega)) (lnWadToRayBody_lt (by omega)) hkey
...
theorem to_wad_eq {x : Nat} (_h : x < 2 ^ 256) :
    lnWadBody x =
      evmSdiv
        (evmSub (lnWadToRayBody x)
          (evmMul (evmSlt (lnWadToRayBody x) 0) 999999999))
        1000000000 := by
  unfold lnWadBody
  simp only
...
theorem lnWadBody_mono {x y : Nat} (hx : 0 < x) (hxy : x ≤ y)
    (hy : y < 2 ^ 255) :
    sle (lnWadBody x) (lnWadBody y) = true := by
  have hx256 : x < 2 ^ 256 := by omega
  have hy256 : y < 2 ^ 256 := by omega
  have hr := lnWadToRayBody_mono hx hxy hy
  have hri := toInt_of_sle (lnWadToRayBody_lt hx256) (lnWadToRayBody_lt hy256) hr
  obtain ⟨hb1, hb2⟩ := lnWadToRayBody_bound hx256
  obtain ⟨hb1', hb2'⟩ := lnWadToRayBody_bound hy256
  have hdd := floordiv_mono (lnWadToRayBody_lt hx256) (lnWadToRayBody_lt hy256) hb1 hb2 hb1' hb2' hri
