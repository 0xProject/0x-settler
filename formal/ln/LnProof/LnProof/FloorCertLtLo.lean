...
theorem ltLo_eval_eq : ∀ x : Int, evalPoly certLtLo x = evalPoly certLtLoLit x := by
  refine evalPoly_ext (B := 50000) certLtLo certLtLoLit ?_ ?_ ?_
  · show polyL1 certLtLo * 2 < 2 ^ 50000
    unfold certLtLo
    ...
    have hfin : (((Sc : Int) * EUD * KF1).natAbs * polyL1 ltTDLit ^ 23 +
        (-(EUD - EUN)).natAbs * (polyL1 ([0, 1] : List Int) * ((23 : Int).natAbs * (LnExp.expNum 22 (polyL1 ltTNLit) (polyL1 ltTDLit) * polyL1 ltTDLit) + (2 : Int).natAbs * polyL1 ltTNLit ^ 23))) * 2 < 2 ^ 50000 := by
      decide +kernel
    have hA := Nat.le_trans h2 h4
    have hB := Nat.le_trans h5 h17
    omega
  · show polyL1 certLtLoLit * 2 < 2 ^ 50000
    decide +kernel
  · show evalPoly certLtLo ((2 : Int) ^ 50000) = evalPoly certLtLoLit ((2 : Int) ^ 50000)
    unfold certLtLo
    rw [ltTN_eq_lit, ltTD_eq_lit]
