...
`lake env lean GenErr1.lean` (after `lake build LnProof.Floor.Consts`). -/

open Common.Exp LnFloor LnYul

#eval do
  let bcap := (Common.Exp.expNum 130 (BIASc * 2 ^ 27) QS * (10 ^ 18 * 10 ^ 42)) / (Common.Exp.fact 130 * QS ^ 130)
  IO.FS.writeFile "LnProof/Cert/BiasCapNum.lean"
