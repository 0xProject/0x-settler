...
library Lib512MathArithmetic {
    ...
    //// Relaxed lnQ256 bounds for 512-bit unsigned integer x.
    ////
    //// The active implementation is a single-stage 16-bucket coarse reduction with a shared
    //// z^5 / [6/7] rational kernel at Q219 precision and G=9 guard bits. It returns:
    ////   - lower in [floor(ln(x) * 2^256) - 1, floor(ln(x) * 2^256)]
    ////   - upper in [ceil(ln(x) * 2^256), ceil(ln(x) * 2^256) + 1]
    ////
