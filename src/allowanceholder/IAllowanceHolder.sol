...
interface IAllowanceHolder {
    ...
    ///         consumed by the `operator` during the execution. Operator consumes the funds during
    ///         its operations by calling back into `AllowanceHolder` with `transferFrom`, consuming
    ///         a token permit.
    ...
    /// @notice The counterpart to `execute` which allows for the consumption of token permits later
    ///         during execution.
