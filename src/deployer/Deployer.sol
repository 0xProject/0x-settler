...
contract Deployer is IDeployer, ERC1967UUPSUpgradeable, Context, ERC1967TwoStepOwnable, ProxyMultiCall {
    ...
    function next(Feature feature) external view override returns (address) {
        ...
        return Create3.predictLondon(salt(feature, featureInfo.list.lastNonce.incr()));
    }
    ...
    function prev(Feature feature) external view override returns (address) {
        ...
        return Create3.predictLondon(salt(feature, prevNonce));
    }
        ...
        returns (address predicted, Nonce thisNonce)
    {
        ...
        predicted = Create3.predictLondon(salt_);
        emit Transfer(
            prevNonce.isNull() ? address(0) : Create3.predictLondon(salt(feature, prevNonce)),
            predicted,
        ...
        if (Create3.createFromCalldataLondon(salt_, initCode, msg.value) != predicted) {
    ...
    function remove(Feature feature, Nonce nonce) public override returns (bool) {
        ...
        address deployment = Create3.predictLondon(salt(feature, nonce));
        if (updatedHead) {
            emit Transfer(
                deployment,
                newHead.isNull() ? address(0) : Create3.predictLondon(salt(feature, newHead)),
                Feature.unwrap(feature)
            );
        }
    ...
    function removeAll(Feature feature) external override returns (bool) {
        ...
        if (!nonce.isNull()) {
            emit Transfer(Create3.predictLondon(salt(feature, nonce)), address(0), Feature.unwrap(feature));
        }
    ...
    function ownerOf(uint256 tokenId) external view override returns (address) {
        ...
        return Create3.predictLondon(salt(feature, _requireTokenExists(feature)));
    }
