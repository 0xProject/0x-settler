...
library FastHanjiPool {
    function placeMarketOrder(
        ...
        assembly ("memory-safe") {
            let ptr := mload(0x40)
            // Solidity `bool` may be any nonzero value; normalize once for arithmetic use below
            isAsk := lt(0x00, isAsk)
    ...
    function getToken(IHanjiPool pool, bool tokenY) internal view returns (IERC20 result) {
        assembly ("memory-safe") {
            let ptr := mload(0x40)
            // Solidity `bool` may be any nonzero value; normalize for use as shift operand
