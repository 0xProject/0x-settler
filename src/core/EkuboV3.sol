...
abstract contract EkuboV3 is SettlerSwapAbstract {
    ...
    function sellToEkuboV3VIP(
        ...
        assembly ("memory-safe") {
            // We can skip all the checks performed by `abi.decode` because we know that this is the
            // verbatim result from `locked_6416899205` and that `locked_6416899205` encoded the buy
            // token and buy amount correctly.
            buyToken := mload(add(0x60, encodedResult))
            buyAmount := mload(add(0x80, encodedResult))
        }
    }
    ...
    function locked_6416899205(bytes calldata data) private returns (bytes memory) {
        ...
        // Settler. `state.buy.token` will be sent to `recipient`.
        {
            ...
            (IERC20 globalBuyToken, uint256 globalBuyAmount) =
                CompactTake.take(state, notes, uint32(IEkuboCore.withdraw.selector), recipient);
            if (feeOnTransfer) {
