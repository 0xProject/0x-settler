...
interface IAllowanceHolder {
    ...
    /// @dev tx.origin / msg.sender is forwarded to target appended to the msg data (similar to ERC-2771)
    ...
    /// @param owner The owner of tokens to transfer; owner must be tx.origin
    ...
    /// @dev msg.sender is forwarded to `target` by appending it to the msg data (similar to ERC-2771)
    /// @param permits A list of tokens and amounts the caller has authorised to be consumed
    /// @param target A contract to execute operations with `data`
    /// @param data The data to forward to `target`
