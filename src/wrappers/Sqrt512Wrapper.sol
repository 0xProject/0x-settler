...
/// own testing purposes. For model generation, `wrap_sqrt512` and
/// `wrap_osqrtUp` use `tmp()` (fixed address 0) so that the Yul IR's
...
contract Sqrt512Wrapper {
    ...
    function wrap_osqrtUp(uint256 x_hi, uint256 x_lo) external pure returns (uint256, uint256) {
        uint512 x;
