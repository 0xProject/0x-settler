...
library Cbrt {
    ...
    function _cbrt(uint256 x) private pure returns (uint256 z) {
        assembly ("memory-safe") {
            ...
            // multipliers `c`: 149/128, 177/128, and 233/128 are selected by `b mod 3` to balance
            // each octave's worst-case final error. This gives >88 bits of precision after only 5
            // Newton-Raphson iterations. The `or(1, ...)` keeps z ≥ 1 when the shifted estimate is
            // 0.
            let b := sub(255, clz(x))
            z := or(1, shr(7, shl(div(b, 3), add(121, shl(mod(b, 3), 28)))))
