...
import {RobinHoodSettlerMetaTxn} from "./MetaTxn.sol";
...
contract RobinHoodSettlerIntent is SettlerIntent, RobinHoodSettlerMetaTxn {
    constructor(bytes20 gitCommit) RobinHoodSettlerMetaTxn(gitCommit) {}
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data, AllowedSlippage memory slippage)
        internal
        override(RobinHoodSettlerMetaTxn, SettlerBase)
        returns (bool)
    {
    ...
    function _msgSender() internal view override(SettlerIntent, RobinHoodSettlerMetaTxn) returns (address) {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(RobinHoodSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(RobinHoodSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
    ...
    function _fallback(bytes calldata data)
        internal
        virtual
        override(Permit2PaymentAbstract, RobinHoodSettlerMetaTxn)
