...
import {ArcSettlerMetaTxn} from "./MetaTxn.sol";
...
contract ArcSettlerIntent is SettlerIntent, ArcSettlerMetaTxn {
    constructor(bytes20 gitCommit) ArcSettlerMetaTxn(gitCommit) {}
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data, AllowedSlippage memory slippage)
        internal
        override(ArcSettlerMetaTxn, SettlerBase)
        returns (bool)
    {
    ...
    function _msgSender() internal view override(SettlerIntent, ArcSettlerMetaTxn) returns (address) {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(ArcSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(ArcSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
    ...
    function _fallback(bytes calldata data)
        internal
        virtual
        override(Permit2PaymentAbstract, ArcSettlerMetaTxn)
