...
import {TaikoSettlerMetaTxn} from "./MetaTxn.sol";
...
contract TaikoSettlerIntent is SettlerIntent, TaikoSettlerMetaTxn {
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data)
        internal
        override(TaikoSettlerMetaTxn, SettlerBase, SettlerAbstract)
        returns (bool)
    {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(TaikoSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(TaikoSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
