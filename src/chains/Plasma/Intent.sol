...
import {PlasmaSettlerMetaTxn} from "./MetaTxn.sol";
...
contract PlasmaSettlerIntent is SettlerIntent, PlasmaSettlerMetaTxn {
    constructor(bytes20 gitCommit) PlasmaSettlerMetaTxn(gitCommit) {}
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data)
        internal
        override(PlasmaSettlerMetaTxn, SettlerBase, SettlerAbstract)
        returns (bool)
    {
    ...
    function _msgSender() internal view override(SettlerIntent, PlasmaSettlerMetaTxn) returns (address) {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(PlasmaSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(PlasmaSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
