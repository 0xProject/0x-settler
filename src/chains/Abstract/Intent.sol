...
contract AbstractSettlerIntent is SettlerIntent, AbstractSettlerMetaTxn {
    constructor(bytes20 gitCommit) AbstractSettlerMetaTxn(gitCommit) {}
    ...
    function _msgSender() internal view override(SettlerIntent, AbstractSettlerMetaTxn) returns (address) {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(AbstractSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(AbstractSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
