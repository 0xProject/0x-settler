...
import {UnichainSettlerMetaTxn} from "./MetaTxn.sol";
...
contract UnichainSettlerIntent is SettlerIntent, UnichainSettlerMetaTxn {
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data)
        internal
        override(UnichainSettlerMetaTxn, SettlerBase, SettlerAbstract)
        returns (bool)
    {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(UnichainSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(UnichainSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
    ...
    function _fallback(bytes calldata data)
        internal
        virtual
        override(Permit2PaymentAbstract, UnichainSettlerMetaTxn)
