...
import {MonadSettlerMetaTxn} from "./MetaTxn.sol";
...
contract MonadSettlerIntent is SettlerIntent, MonadSettlerMetaTxn {
    constructor(bytes20 gitCommit) MonadSettlerMetaTxn(gitCommit) {}
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data)
        internal
        override(MonadSettlerMetaTxn, SettlerBase, SettlerAbstract)
        returns (bool)
    {
    ...
    function _msgSender() internal view override(SettlerIntent, MonadSettlerMetaTxn) returns (address) {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(MonadSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(MonadSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
