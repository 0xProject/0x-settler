...
import {SonicSettlerMetaTxn} from "./MetaTxn.sol";
...
contract SonicSettlerIntent is SettlerIntent, SonicSettlerMetaTxn {
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data)
        internal
        override(SonicSettlerMetaTxn, SettlerBase, SettlerAbstract)
        returns (bool)
    {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(SonicSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
    ...
    function _isRestrictedTarget(address target)
        ...
        view
        virtual
        override(SonicSettlerMetaTxn, SettlerIntent)
        returns (bool)
    {
