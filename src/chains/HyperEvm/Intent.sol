...
contract HyperEvmSettlerIntent is SettlerIntent, HyperEvmSettlerMetaTxn {
    constructor(bytes20 gitCommit) HyperEvmSettlerMetaTxn(gitCommit) {}
    ...
    function _dispatch(uint256 i, uint256 action, bytes calldata data)
        internal
        override(HyperEvmSettlerMetaTxn, SettlerBase, SettlerAbstract)
        returns (bool)
    {
    ...
    function _msgSender() internal view override(SettlerIntent, HyperEvmSettlerMetaTxn) returns (address) {
    ...
    function _dispatchVIP(uint256 action, bytes calldata data, bytes calldata sig)
        internal
        override(HyperEvmSettlerMetaTxn, SettlerMetaTxn)
        returns (bool)
    {
