...
if [[ ${BROADCAST-no} = [Yy]es ]] ; then
    ...
    # Buffer jq's output before iterating: piping `jq | while read` via process substitution would
    # emit valid prefix rows to stdout before failing, leaving sender_gas partially populated and
    # silently broadcasting an unchecked sender. `set -e` aborts on the failed substitution.
    declare jq_out
    jq_out="$(jq -r '.transactions[] | [.transaction.from // error, .transaction.gas // error] | @tsv' "$dry_run_json")"
    ...
    for sender in "${!sender_gas[@]}" ; do
        # `sender_gas` already includes `gas_estimate_multiplier` (forge applies it during simulation
        # before serializing tx.gas), so don't multiply again. Execution-gas floor only (excludes L2
        # L1-data fees): a coarse "grossly underfunded" guard, not an exact estimate.
