...
deploy_calldata="$(cast concat-hex 0x0000000000000000000000000000000000000009435af220071616d150499b5f "$(forge inspect src/CrossChainReceiverFactory.sol:CrossChainReceiverFactory bytecode)")"
...
declare -i gas_limit
gas_limit="$(cast estimate --from "$signer" --rpc-url "$rpc_url" --gas-price $gas_price --chain $chainid "$toehold" "$deploy_calldata")"
...
cast "${maybe_broadcast[@]}" --from "$signer" --rpc-url "$submit_rpc" --gas-price $gas_price --gas-limit $gas_limit $(get_config extraFlags) "$toehold" "$deploy_calldata"
